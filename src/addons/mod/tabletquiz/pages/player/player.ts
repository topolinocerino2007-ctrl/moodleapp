...
import { AddonModTabletTabletQuizAutoSave } from '../../classes/auto-save';
import {
    AddonModTabletTabletQuizNavigationModalReturn,
    AddonModTabletTabletQuizNavigationQuestion,
...
import { AddonModTabletTabletQuizHelper } from '../../services/tabletquiz-helper';
import { AddonModTabletTabletQuizSync } from '../../services/tabletquiz-sync';
...
import { AddonModTabletTabletQuizQuestionCardComponent } from '../../components/question-card/question-card';
import { CoreSharedModule } from '@/core/shared.module';

/**
 * Page that allows attempting a tabletquiz.
 */
@Component({
    selector: 'page-addon-mod-tablettabletquiz-player',
    templateUrl: 'player.html',
    styleUrl: 'player.scss',
    imports: [
        CoreSharedModule,
        AddonModTabletTabletQuizQuestionCardComponent,
        CoreQuestionComponent,
    ],
})
export default class AddonModTabletTabletQuizPlayerPage implements OnInit, OnDestroy, CanLeave {

    readonly content = viewChild.required(IonContent);
    readonly questionComponents = viewChildren(CoreQuestionComponent);
    readonly formElement = viewChild<ElementRef>('tabletquizForm');

    tabletquiz?: AddonModTabletTabletQuizTabletQuizWSData; // The tabletquiz the attempt belongs to.
    attempt?: TabletQuizAttempt; // The attempt being attempted.
    moduleUrl?: string; // URL to the module in the site.
    component = ADDON_MOD_TABLETQUIZ_COMPONENT_LEGACY; // Component to link the files to.
    loaded = false; // Whether data has been loaded.
    tabletquizAborted = false; // Whether the tabletquiz was aborted due to an error.
    offline = false; // Whether the tabletquiz is being attempted in offline mode.
    attemptSummary: AddonModTabletTabletQuizNavigationQuestion[] = []; // Attempt summary: list of questions to navigate.
    ...
    preventSubmitMessages: string[] = []; // List of messages explaining why the tabletquiz cannot be submitted.
    endTime?: number; // The time when the attempt must be finished.
    autoSaveError = false; // Whether there's been an error in auto-save.
    isSequential = false; // Whether tabletquiz navigation is sequential.
    readableTimeLimit?: string; // Time limit in a readable format.
    dueDateWarning?: string; // Warning about due date.
    courseId!: number; // The course ID the tabletquiz belongs to.
    cmId!: number; // Course module ID.
    correctIcon = '';
    incorrectIcon = '';
    partialCorrectIcon = '';

    protected preflightData: Record<string, string> = {}; // Preflight data to attempt the tabletquiz.
    protected tabletquizAccessInfo?: AddonModTabletTabletQuizGetTabletQuizAccessInformationWSResponse; // TabletQuiz access information.
    protected attemptAccessInfo?: AddonModTabletTabletQuizGetAttemptAccessInformationWSResponse; // Attempt access info.
    protected lastAttempt?: TabletQuizAttempt; // Last user attempt before a new one is created (if needed).
    protected newAttempt = false; // Whether the user is starting a new attempt.
    protected tabletquizDataLoaded = false; // Whether the tabletquiz data has been loaded.
    protected timeUpCalled = false; // Whether the time up function has been called.
    protected autoSave!: AddonModTabletTabletQuizAutoSave; // Class to auto-save answers every certain time.
    ...
    ngOnInit(): void {
        ...
        this.autoSave = new AddonModTabletTabletQuizAutoSave(
            'addon-mod_tabletquiz-player-form',
            '#addon-mod_tabletquiz-connection-error-button',
        );
    ...
    ngOnDestroy(): void {
        ...
        if (this.tabletquiz) {
            // Unblock the tabletquiz so it can be synced.
            CoreSync.unblockOperation(ADDON_MOD_TABLETQUIZ_COMPONENT, this.tabletquiz.id);
        }
    }
    ...
    async canLeave(): Promise<boolean> {
        if (this.forceLeave || this.tabletquizAborted || !this.questions.length || this.showSummary) {
            return true;
        }
        ...
        try {
            ...
            await CoreAlerts.confirm(Translate.instant('addon.mod_tablettabletquiz.confirmleavetabletquizonerror'));
    ...
    /**
     * Abort the tabletquiz.
     */
    abortTabletQuiz(): void {
        this.tabletquizAborted = true;
    }
    ...
    async behaviourButtonClicked(button: CoreQuestionBehaviourButton): Promise<void> {
        if (!this.tabletquiz || !this.attempt) {
            return;
        }
        ...
        try {
            ...
            // Get the answers.
            const answers = await this.prepareAnswers(this.tabletquiz.coursemodule);

            // Add the clicked button data.
            answers[button.name] = button.value;

            // Behaviour checks are always in online.
            await AddonModTabletTabletQuiz.processAttempt(this.tabletquiz, this.attempt, answers, this.preflightData);
    ...
    async changePage(page: number, fromModal?: boolean, slot?: number): Promise<void> {
        ...
        if (page !== -1 && (this.attempt.state === AddonModTabletTabletQuizAttemptStates.OVERDUE || this.attempt.finishedOffline)) {
            ...
            // Also, in sequential tabletquizzes we can only navigate to the current page.
            return;
        ...
        if (!this.showSummary) {
            ...
            try {
                ...
                CoreAlerts.showError(error, { default: Translate.instant('addon.mod_tablettabletquiz.errorsaveattempt') });
                modal.dismiss();
        ...
        try {
            ...
            if (!this.showSummary && this.tabletquiz) {
                this.autoSave.startCheckChangesProcess(this.tabletquiz, this.attempt, this.preflightData, this.offline);
            }

            CoreAlerts.showError(error, { default: Translate.instant('addon.mod_tablettabletquiz.errorgetquestions') });
        } finally {
            this.loaded = true;
    ...
    /**
     * Convenience function to get the tabletquiz data.
     */
    protected async fetchData(): Promise<void> {
        this.tabletquiz = await AddonModTabletTabletQuiz.getTabletQuiz(this.courseId, this.cmId);

        // Block the tabletquiz so it cannot be synced.
        CoreSync.blockOperation(ADDON_MOD_TABLETQUIZ_COMPONENT, this.tabletquiz.id);

        // Wait for any ongoing sync to finish. We won't sync a tabletquiz while it's being played.
        await AddonModTabletTabletQuizSync.waitForSync(this.tabletquiz.id);

        this.isSequential = AddonModTabletTabletQuiz.isNavigationSequential(this.tabletquiz);

        if (AddonModTabletTabletQuiz.isTabletQuizOffline(this.tabletquiz)) {
            // TabletQuiz supports offline.
            this.offline = true;
        } else {
            // TabletQuiz doesn't support offline right now, but maybe it did and then the setting was changed.
            // If we have an unfinished offline attempt then we'll use offline mode.
            this.offline = await AddonModTabletTabletQuiz.isLastAttemptOfflineUnfinished(this.tabletquiz);
        }

        if (this.tabletquiz.timelimit && this.tabletquiz.timelimit > 0) {
            this.readableTimeLimit = CoreTime.formatTime(this.tabletquiz.timelimit);
        }

        // Get access information for the tabletquiz.
        this.tabletquizAccessInfo = await AddonModTabletTabletQuiz.getTabletQuizAccessInformation(this.tabletquiz.id, {
        ...
        const attempts = await AddonModTabletTabletQuiz.getUserAttempts(this.tabletquiz.id, {
            cmId: this.tabletquiz.coursemodule,
            readingStrategy: this.offline ? CoreSitesReadingStrategy.PREFER_CACHE : CoreSitesReadingStrategy.ONLY_NETWORK,
        });
        ...
        this.lastAttempt.finishedOffline = await AddonModTabletTabletQuiz.isAttemptFinishedOffline(this.lastAttempt.id);

        this.newAttempt = AddonModTabletTabletQuiz.isAttemptCompleted(this.lastAttempt.state);
    }
     ...
     * @param timeUp Whether the tabletquiz time is up.
     */
    async finishAttempt(userFinish?: boolean, timeUp?: boolean): Promise<void> {
        ...
        try {
            // Show confirm if the user clicked the finish button and the tabletquiz is in progress.
            if (!timeUp && this.attempt.state === AddonModTabletTabletQuizAttemptStates.IN_PROGRESS) {
                let message = Translate.instant('addon.mod_tablettabletquiz.confirmclose');

                const unansweredCount = this.attemptSummary
                    .filter(question => AddonModTabletTabletQuiz.isQuestionUnanswered(question))
                    .length;

                if (!this.isSequential && unansweredCount > 0) {
                    const warning = Translate.instant(
                        'addon.mod_tablettabletquiz.submission_confirmation_unanswered',
                        { $a: unansweredCount },
                    );
                ...
                await CoreAlerts.confirm(message, {
                    header: Translate.instant('addon.mod_tablettabletquiz.submitallandfinish'),
            ...
            CoreEvents.trigger(ADDON_MOD_TABLETQUIZ_ATTEMPT_FINISHED_EVENT, {
                tabletquizId: this.tabletquiz.id,
                attemptId: this.attempt.id,
                synced: !this.offline,
            }, CoreSites.getCurrentSiteId());

            CoreEvents.trigger(CoreEvents.ACTIVITY_DATA_SENT, { module: 'tabletquiz' });

            if (!timeUp || !this.tabletquiz.graceperiod) {
                // Leave the player.
            ...
            CoreAlerts
                .showError(error, { default: Translate.instant('addon.mod_tablettabletquiz.errorsaveattempt') })
                .then(async alert => {
    ...
    protected async fixSequenceChecks(): Promise<void> {
        ...
        const data = await AddonModTabletTabletQuiz.getAttemptData(this.attempt.id, this.attempt.currentpage ?? 0, this.preflightData, {
            cmId: this.tabletquiz?.coursemodule,
            readingStrategy: this.offline ? CoreSitesReadingStrategy.PREFER_CACHE : CoreSitesReadingStrategy.ONLY_NETWORK,
        });
    ...
    protected getAnswers(): CoreQuestionsAnswers {
        return CoreQuestionHelper.getAnswersFromForm(document.forms['addon-mod_tabletquiz-player-form']);
    }
    ...
    protected initTimer(): void {
        if (!this.tabletquizAccessInfo || !this.attempt || !this.attemptAccessInfo?.endtime || this.attemptAccessInfo.endtime < 0) {
            return;
        }

        // TabletQuiz has an end time. Check if time left should be shown.
        const shouldShowTime = AddonModTabletTabletQuiz.shouldShowTimeLeft(
            this.tabletquizAccessInfo.activerulenames,
            this.attempt,
    ...
    protected async loadPage(page: number): Promise<void> {
        ...
        const data = await AddonModTabletTabletQuiz.getAttemptData(this.attempt.id, page, this.preflightData, {
            cmId: this.tabletquiz.coursemodule,
            readingStrategy: this.offline ? CoreSitesReadingStrategy.PREFER_CACHE : CoreSitesReadingStrategy.ONLY_NETWORK,
        });
        ...
        this.questions.forEach((question) => {
            ...
            question.readableMark = AddonModTabletTabletQuizHelper.getQuestionMarkFromHtml(question.html);
            ...
            if (AddonModTabletTabletQuiz.isQuestionBlocked(question)) {
    ...
    protected async logViewPage(page: number): Promise<void> {
        ...
        await CorePromiseUtils.ignoreErrors(AddonModTabletTabletQuiz.logViewAttempt(this.attempt.id, page, this.preflightData, this.offline));

        CoreAnalytics.logEvent({
            type: CoreAnalyticsEventType.VIEW_ITEM,
            ws: 'mod_tablettabletquiz_view_attempt',
            name: this.tabletquiz.name,
            data: { id: this.attempt.id, tabletquizid: this.tabletquiz.id, page, category: 'tabletquiz' },
            url: `/mod/tabletquiz/attempt.php?attempt=${this.attempt.id}&cmid=${this.cmId}` + (page > 0 ? `&page=${page}` : ''),
        });
    }
    ...
    protected async logViewSummary(): Promise<void> {
        ...
        await CorePromiseUtils.ignoreErrors(
            AddonModTabletTabletQuiz.logViewAttemptSummary(this.attempt.id, this.preflightData, this.tabletquiz.id),
        );

        CoreAnalytics.logEvent({
            type: CoreAnalyticsEventType.VIEW_ITEM,
            ws: 'mod_tablettabletquiz_view_attempt_summary',
            name: this.tabletquiz.name,
            data: { id: this.attempt.id, tabletquizid: this.tabletquiz.id, category: 'tabletquiz' },
            url: `/mod/tabletquiz/summary.php?attempt=${this.attempt.id}&cmid=${this.cmId}`,
        });
    }
    ...
    protected async loadSummary(): Promise<void> {
        ...
        this.canReturn = this.attempt.state === AddonModTabletTabletQuizAttemptStates.IN_PROGRESS && !this.attempt.finishedOffline;
        this.preventSubmitMessages = AddonModTabletTabletQuiz.getPreventSubmitMessages(this.attemptSummary);

        this.dueDateWarning = AddonModTabletTabletQuiz.getAttemptDueDateWarning(this.tabletquiz, this.attempt);
    ...
    protected async loadAttemptSummary(): Promise<void> {
        ...
        this.attemptSummary = await AddonModTabletTabletQuiz.getAttemptSummary(this.attempt.id, this.preflightData, {
            cmId: this.tabletquiz.coursemodule,
    ...
    async openNavigation(): Promise<void> {
        ...
        const { AddonModTabletTabletQuizNavigationModalComponent } = await import('../../components/navigation-modal/navigation-modal');

        // Create the navigation modal.
        const modalData = await CoreModals.openSideModal<AddonModTabletTabletQuizNavigationModalReturn>({
            component: AddonModTabletTabletQuizNavigationModalComponent,
            componentProps: {
    ...
    protected async processAttempt(userFinish?: boolean, timeUp?: boolean, retrying?: boolean): Promise<void> {
        ...
        if (!this.showSummary) {
            answers = await this.prepareAnswers(this.tabletquiz.coursemodule);
        }

        try {
            // Send the answers.
            await AddonModTabletTabletQuiz.processAttempt(
                this.tabletquiz,
                this.attempt,
                answers,
                this.preflightData,
        ...
        await CoreQuestionHelper.clearTmpData(this.questions, this.component, this.tabletquiz.coursemodule);
    }
    ...
    protected async scrollToQuestion(slot: number): Promise<void> {
        ...
        await CoreDom.scrollToElement(
            this.element,
            `#addon-mod_tabletquiz-question-${slot}`,
        );
    }
    ...
    async start(): Promise<void> {
        try {
            ...
            if (!this.tabletquizDataLoaded) {
                // Fetch data.
                await this.fetchData();

                this.tabletquizDataLoaded = true;
            }

            // TabletQuiz data has been loaded, try to start or continue.
            await this.startOrContinueAttempt();
        } catch (error) {
            CoreAlerts.showError(error, { default: Translate.instant('addon.mod_tablettabletquiz.errorgettabletquiz') });
        } finally {
            this.loaded = true;
        }
    }
    ...
    protected async startOrContinueAttempt(): Promise<void> {
        if (!this.tabletquiz || !this.tabletquizAccessInfo) {
            return;
        }
        ...
        attempt = await AddonModTabletTabletQuizHelper.getAndCheckPreflightData(
            this.tabletquiz,
            this.tabletquizAccessInfo,
            this.preflightData,
            {
                attempt,
                offline: this.offline,
                finishedOffline: attempt?.finishedOffline,
                title: 'addon.mod_tablettabletquiz.startattempt',
            },
        );

        // Re-fetch attempt access information with the right attempt (might have changed because a new attempt was created).
        this.attemptAccessInfo = await AddonModTabletTabletQuiz.getAttemptAccessInformation(this.tabletquiz.id, attempt.id, {
            cmId: this.tabletquiz.coursemodule,
            readingStrategy: this.offline ? CoreSitesReadingStrategy.PREFER_CACHE : CoreSitesReadingStrategy.ONLY_NETWORK,
        });
        ...
        if (this.attempt.state !== AddonModTabletTabletQuizAttemptStates.OVERDUE && !this.attempt.finishedOffline) {
 ...
 */
type TabletQuizAttempt = AddonModTabletTabletQuizAttemptWSData & {
