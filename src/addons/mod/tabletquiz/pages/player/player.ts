...
import { AddonModTabletQuizAutoSave } from '../../classes/auto-save';
import {
    AddonModTabletQuizNavigationModalReturn,
    AddonModTabletQuizNavigationQuestion,
...
import { AddonModTabletQuizHelper } from '../../services/tabletquiz-helper';
import { AddonModTabletQuizSync } from '../../services/tabletquiz-sync';
...
import { AddonModTabletQuizQuestionCardComponent } from '../../components/question-card/question-card';
 ...
 */
@Component({
    selector: 'page-addon-mod-tabletquiz-player',
    templateUrl: 'player.html',
    styleUrl: 'player.scss',
    imports: [
        CoreSharedModule,
        AddonModTabletQuizQuestionCardComponent,
        CoreQuestionComponent,
    ],
})
export default class AddonModTabletQuizPlayerPage implements OnInit, OnDestroy, CanLeave {
    ...
    tabletquiz?: AddonModTabletQuizTabletQuizWSData; // The tabletquiz the attempt belongs to.
    ...
    attemptSummary: AddonModTabletQuizNavigationQuestion[] = []; // Attempt summary: list of questions to navigate.
    ...
    protected tabletquizAccessInfo?: AddonModTabletQuizGetTabletQuizAccessInformationWSResponse; // TabletQuiz access information.
    protected attemptAccessInfo?: AddonModTabletQuizGetAttemptAccessInformationWSResponse; // Attempt access info.
    ...
    protected autoSave!: AddonModTabletQuizAutoSave; // Class to auto-save answers every certain time.
    ...
    ngOnInit(): void {
        ...
        this.autoSave = new AddonModTabletQuizAutoSave(
    ...
    async canLeave(): Promise<boolean> {
        ...
        try {
            ...
            await CoreAlerts.confirm(Translate.instant('addon.mod_tabletquiz.confirmleavetabletquizonerror'));
    ...
    async behaviourButtonClicked(button: CoreQuestionBehaviourButton): Promise<void> {
        ...
        try {
            ...
            await AddonModTabletQuiz.processAttempt(this.tabletquiz, this.attempt, answers, this.preflightData);
    ...
    async changePage(page: number, fromModal?: boolean, slot?: number): Promise<void> {
        ...
        if (page !== -1 && (this.attempt.state === AddonModTabletQuizAttemptStates.OVERDUE || this.attempt.finishedOffline)) {
        ...
        if (!this.showSummary) {
            ...
            try {
                ...
                CoreAlerts.showError(error, { default: Translate.instant('addon.mod_tabletquiz.errorsaveattempt') });
                modal.dismiss();
        ...
        try {
            ...
            CoreAlerts.showError(error, { default: Translate.instant('addon.mod_tabletquiz.errorgetquestions') });
        } finally {
            this.loaded = true;
    ...
    protected async fetchData(): Promise<void> {
        this.tabletquiz = await AddonModTabletQuiz.getTabletQuiz(this.courseId, this.cmId);
        ...
        await AddonModTabletQuizSync.waitForSync(this.tabletquiz.id);

        this.isSequential = AddonModTabletQuiz.isNavigationSequential(this.tabletquiz);
        ...
        if (AddonModTabletQuiz.isTabletQuizOffline(this.tabletquiz)) {
            ...
            this.offline = await AddonModTabletQuiz.isLastAttemptOfflineUnfinished(this.tabletquiz);
        }
        ...
        this.tabletquizAccessInfo = await AddonModTabletQuiz.getTabletQuizAccessInformation(this.tabletquiz.id, {
        ...
        const attempts = await AddonModTabletQuiz.getUserAttempts(this.tabletquiz.id, {
            cmId: this.tabletquiz.coursemodule,
            readingStrategy: this.offline ? CoreSitesReadingStrategy.PREFER_CACHE : CoreSitesReadingStrategy.ONLY_NETWORK,
        });
        ...
        this.lastAttempt.finishedOffline = await AddonModTabletQuiz.isAttemptFinishedOffline(this.lastAttempt.id);

        this.newAttempt = AddonModTabletQuiz.isAttemptCompleted(this.lastAttempt.state);
    }
    ...
    async finishAttempt(userFinish?: boolean, timeUp?: boolean): Promise<void> {
        ...
        try {
            ...
            if (!timeUp && this.attempt.state === AddonModTabletQuizAttemptStates.IN_PROGRESS) {
                let message = Translate.instant('addon.mod_tabletquiz.confirmclose');

                const unansweredCount = this.attemptSummary
                    .filter(question => AddonModTabletQuiz.isQuestionUnanswered(question))
                    .length;

                if (!this.isSequential && unansweredCount > 0) {
                    const warning = Translate.instant(
                        'addon.mod_tabletquiz.submission_confirmation_unanswered',
                        { $a: unansweredCount },
                    );
                ...
                await CoreAlerts.confirm(message, {
                    header: Translate.instant('addon.mod_tabletquiz.submitallandfinish'),
            ...
            CoreAlerts
                .showError(error, { default: Translate.instant('addon.mod_tabletquiz.errorsaveattempt') })
                .then(async alert => {
    ...
    protected async fixSequenceChecks(): Promise<void> {
        ...
        const data = await AddonModTabletQuiz.getAttemptData(this.attempt.id, this.attempt.currentpage ?? 0, this.preflightData, {
            cmId: this.tabletquiz?.coursemodule,
            readingStrategy: this.offline ? CoreSitesReadingStrategy.PREFER_CACHE : CoreSitesReadingStrategy.ONLY_NETWORK,
        });
    ...
    protected initTimer(): void {
        ...
        const shouldShowTime = AddonModTabletQuiz.shouldShowTimeLeft(
    ...
    protected async loadPage(page: number): Promise<void> {
        ...
        const data = await AddonModTabletQuiz.getAttemptData(this.attempt.id, page, this.preflightData, {
            cmId: this.tabletquiz.coursemodule,
            readingStrategy: this.offline ? CoreSitesReadingStrategy.PREFER_CACHE : CoreSitesReadingStrategy.ONLY_NETWORK,
        });
        ...
        this.questions.forEach((question) => {
            ...
            question.readableMark = AddonModTabletQuizHelper.getQuestionMarkFromHtml(question.html);
            ...
            if (AddonModTabletQuiz.isQuestionBlocked(question)) {
    ...
    protected async logViewPage(page: number): Promise<void> {
        ...
        await CorePromiseUtils.ignoreErrors(AddonModTabletQuiz.logViewAttempt(this.attempt.id, page, this.preflightData, this.offline));

        CoreAnalytics.logEvent({
            type: CoreAnalyticsEventType.VIEW_ITEM,
            ws: 'mod_tabletquiz_view_attempt',
    ...
    protected async logViewSummary(): Promise<void> {
        ...
        await CorePromiseUtils.ignoreErrors(
            AddonModTabletQuiz.logViewAttemptSummary(this.attempt.id, this.preflightData, this.tabletquiz.id),
        );

        CoreAnalytics.logEvent({
            type: CoreAnalyticsEventType.VIEW_ITEM,
            ws: 'mod_tabletquiz_view_attempt_summary',
            name: this.tabletquiz.name,
    ...
    protected async loadSummary(): Promise<void> {
        ...
        this.canReturn = this.attempt.state === AddonModTabletQuizAttemptStates.IN_PROGRESS && !this.attempt.finishedOffline;
        this.preventSubmitMessages = AddonModTabletQuiz.getPreventSubmitMessages(this.attemptSummary);

        this.dueDateWarning = AddonModTabletQuiz.getAttemptDueDateWarning(this.tabletquiz, this.attempt);
    ...
    protected async loadAttemptSummary(): Promise<void> {
        ...
        this.attemptSummary = await AddonModTabletQuiz.getAttemptSummary(this.attempt.id, this.preflightData, {
            cmId: this.tabletquiz.coursemodule,
    ...
    async openNavigation(): Promise<void> {
        ...
        const { AddonModTabletQuizNavigationModalComponent } = await import('../../components/navigation-modal/navigation-modal');

        // Create the navigation modal.
        const modalData = await CoreModals.openSideModal<AddonModTabletQuizNavigationModalReturn>({
            component: AddonModTabletQuizNavigationModalComponent,
            componentProps: {
    ...
    protected async processAttempt(userFinish?: boolean, timeUp?: boolean, retrying?: boolean): Promise<void> {
        ...
        try {
            // Send the answers.
            await AddonModTabletQuiz.processAttempt(
                this.tabletquiz,
                this.attempt,
                answers,
                this.preflightData,
    ...
    async start(): Promise<void> {
        try {
            ...
            CoreAlerts.showError(error, { default: Translate.instant('addon.mod_tabletquiz.errorgettabletquiz') });
        } finally {
            this.loaded = true;
        }
    }
    ...
    protected async startOrContinueAttempt(): Promise<void> {
        ...
        attempt = await AddonModTabletQuizHelper.getAndCheckPreflightData(
            this.tabletquiz,
            ...
            this.preflightData,
            {
                ...
                title: 'addon.mod_tabletquiz.startattempt',
            },
        );

        // Re-fetch attempt access information with the right attempt (might have changed because a new attempt was created).
        this.attemptAccessInfo = await AddonModTabletQuiz.getAttemptAccessInformation(this.tabletquiz.id, attempt.id, {
            cmId: this.tabletquiz.coursemodule,
            readingStrategy: this.offline ? CoreSitesReadingStrategy.PREFER_CACHE : CoreSitesReadingStrategy.ONLY_NETWORK,
        });
        ...
        if (this.attempt.state !== AddonModTabletQuizAttemptStates.OVERDUE && !this.attempt.finishedOffline) {
 ...
 */
type TabletQuizAttempt = AddonModTabletQuizAttemptWSData & {
