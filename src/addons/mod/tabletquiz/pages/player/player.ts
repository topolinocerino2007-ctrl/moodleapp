...
export default class AddonModTabletQuizPlayerPage implements OnInit, OnDestroy, CanLeave {
    ...
    TabletQuiz?: AddonModTabletQuizQuizWSData; // The TabletQuiz the attempt belongs to.
    ...
    quizAborted = false; // Whether the TabletQuiz was aborted due to an error.
    ...
    protected quizAccessInfo?: AddonModTabletQuizGetQuizAccessInformationWSResponse; // TabletQuiz access information.
    ...
    protected quizDataLoaded = false; // Whether the TabletQuiz data has been loaded.
    ...
    async changePage(page: number, fromModal?: boolean, slot?: number): Promise<void> {
        ...
        if (!this.showSummary) {
            ...
            try {
                ...
                CoreAlerts.showError(error, { default: Translate.instant('addon.mod_tabletquiz.errorsavetabletquizattempt') });
                modal.dismiss();
    ...
    protected async fetchData(): Promise<void> {
        this.TabletQuiz = await AddonModTabletQuiz.getQuiz(this.courseId, this.cmId);
        ...
        if (AddonModTabletQuiz.isQuizOffline(this.TabletQuiz)) {
        ...
        this.quizAccessInfo = await AddonModTabletQuiz.getQuizAccessInformation(this.TabletQuiz.id, {
    ...
    async finishAttempt(userFinish?: boolean, timeUp?: boolean): Promise<void> {
        ...
        try {
            ...
            CoreAlerts
                .showError(error, { default: Translate.instant('addon.mod_tabletquiz.errorsavetabletquizattempt') })
                .then(async alert => {
    ...
    protected async logViewPage(page: number): Promise<void> {
        ...
        CoreAnalytics.logEvent({
            ...
            data: { id: this.attempt.id, quizid: this.TabletQuiz.id, page, category: 'tabletquiz' },
    ...
    protected async logViewSummary(): Promise<void> {
        ...
        CoreAnalytics.logEvent({
            ...
            name: this.TabletQuiz.name,
            data: { id: this.attempt.id, quizid: this.TabletQuiz.id, category: 'tabletquiz' },
    ...
    protected async startOrContinueAttempt(): Promise<void> {
        if (!this.TabletQuiz || !this.quizAccessInfo) {
            return;
        }
 ...
 */
type QuizAttempt = AddonModTabletQuizAttemptWSData & {
