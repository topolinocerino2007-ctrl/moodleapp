...
import { AddonModTabletQuizHelper } from '../../services/quiz-helper';
import { AddonModTabletQuizSync } from '../../services/quiz-sync';
...
export default class AddonModTabletQuizPlayerPage implements OnInit, OnDestroy, CanLeave {
    ...
    quiz?: AddonModTabletQuizQuizWSData; // The quiz the attempt belongs to.
    ...
    protected quizAccessInfo?: AddonModTabletQuizGetQuizAccessInformationWSResponse; // Quiz access information.
    ...
    ngOnDestroy(): void {
        ...
        if (this.quiz) {
            ...
            CoreSync.unblockOperation(ADDON_MOD_TABLETQUIZ_COMPONENT, this.quiz.id);
        }
    }
    ...
    async canLeave(): Promise<boolean> {
        ...
        try {
            ...
            await CoreAlerts.confirm(Translate.instant('addon.mod_tabletquiz.confirmleavequizonerror'));
    ...
    async behaviourButtonClicked(button: CoreQuestionBehaviourButton): Promise<void> {
        ...
        try {
            ...
            await AddonModTabletQuiz.processAttempt(this.quiz, this.attempt, answers, this.preflightData);
    ...
    protected async fetchData(): Promise<void> {
        this.quiz = await AddonModTabletQuiz.getQuiz(this.courseId, this.cmId);

        // Block the quiz so it cannot be synced.
        CoreSync.blockOperation(ADDON_MOD_TABLETQUIZ_COMPONENT, this.quiz.id);

        // Wait for any ongoing sync to finish. We won't sync a quiz while it's being played.
        await AddonModTabletQuizSync.waitForSync(this.quiz.id);

        this.isSequential = AddonModTabletQuiz.isNavigationSequential(this.quiz);

        if (AddonModTabletQuiz.isQuizOffline(this.quiz)) {
            ...
            this.offline = await AddonModTabletQuiz.isLastAttemptOfflineUnfinished(this.quiz);
        }
        ...
        this.quizAccessInfo = await AddonModTabletQuiz.getQuizAccessInformation(this.quiz.id, {
        ...
        const attempts = await AddonModTabletQuiz.getUserAttempts(this.quiz.id, {
            cmId: this.quiz.coursemodule,
            readingStrategy: this.offline ? CoreSitesReadingStrategy.PREFER_CACHE : CoreSitesReadingStrategy.ONLY_NETWORK,
        });
    ...
    protected async logViewSummary(): Promise<void> {
        ...
        await CorePromiseUtils.ignoreErrors(
            AddonModTabletQuiz.logViewAttemptSummary(this.attempt.id, this.preflightData, this.quiz.id),
        );
    ...
    protected async loadSummary(): Promise<void> {
        ...
        this.dueDateWarning = AddonModTabletQuiz.getAttemptDueDateWarning(this.quiz, this.attempt);
    ...
    async start(): Promise<void> {
        try {
            ...
            CoreAlerts.showError(error, { default: Translate.instant('addon.mod_tabletquiz.errorgetquiz') });
        } finally {
            this.loaded = true;
        }
    }
    ...
    protected async startOrContinueAttempt(): Promise<void> {
        ...
        this.attemptAccessInfo = await AddonModTabletQuiz.getAttemptAccessInformation(this.quiz.id, attempt.id, {
            cmId: this.quiz.coursemodule,
            readingStrategy: this.offline ? CoreSitesReadingStrategy.PREFER_CACHE : CoreSitesReadingStrategy.ONLY_NETWORK,
        });
