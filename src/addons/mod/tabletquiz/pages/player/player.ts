...
import { AddonModQuizHelper } from '../../services/TabletQuiz-helper';
import { AddonModQuizSync } from '../../services/TabletQuiz-sync';
...
export default class AddonModQuizPlayerPage implements OnInit, OnDestroy, CanLeave {
    ...
    TabletQuiz?: AddonModQuizQuizWSData; // The TabletQuiz the attempt belongs to.
    ...
    protected quizAccessInfo?: AddonModQuizGetQuizAccessInformationWSResponse; // TabletQuiz access information.
    ...
    ngOnDestroy(): void {
        ...
        if (this.TabletQuiz) {
            ...
            CoreSync.unblockOperation(ADDON_MOD_QUIZ_COMPONENT, this.TabletQuiz.id);
        }
    }
    ...
    async behaviourButtonClicked(button: CoreQuestionBehaviourButton): Promise<void> {
        ...
        try {
            ...
            await AddonModQuiz.processAttempt(this.TabletQuiz, this.attempt, answers, this.preflightData);
    ...
    protected async fetchData(): Promise<void> {
        this.TabletQuiz = await AddonModQuiz.getQuiz(this.courseId, this.cmId);

        // Block the TabletQuiz so it cannot be synced.
        CoreSync.blockOperation(ADDON_MOD_QUIZ_COMPONENT, this.TabletQuiz.id);

        // Wait for any ongoing sync to finish. We won't sync a TabletQuiz while it's being played.
        await AddonModQuizSync.waitForSync(this.TabletQuiz.id);

        this.isSequential = AddonModQuiz.isNavigationSequential(this.TabletQuiz);

        if (AddonModQuiz.isQuizOffline(this.TabletQuiz)) {
            ...
            this.offline = await AddonModQuiz.isLastAttemptOfflineUnfinished(this.TabletQuiz);
        }
        ...
        this.quizAccessInfo = await AddonModQuiz.getQuizAccessInformation(this.TabletQuiz.id, {
        ...
        const attempts = await AddonModQuiz.getUserAttempts(this.TabletQuiz.id, {
            cmId: this.TabletQuiz.coursemodule,
            readingStrategy: this.offline ? CoreSitesReadingStrategy.PREFER_CACHE : CoreSitesReadingStrategy.ONLY_NETWORK,
        });
    ...
    protected async logViewSummary(): Promise<void> {
        ...
        await CorePromiseUtils.ignoreErrors(
            AddonModQuiz.logViewAttemptSummary(this.attempt.id, this.preflightData, this.TabletQuiz.id),
        );
    ...
    protected async loadSummary(): Promise<void> {
        ...
        this.dueDateWarning = AddonModQuiz.getAttemptDueDateWarning(this.TabletQuiz, this.attempt);
    ...
    protected async startOrContinueAttempt(): Promise<void> {
        ...
        this.attemptAccessInfo = await AddonModQuiz.getAttemptAccessInformation(this.TabletQuiz.id, attempt.id, {
            cmId: this.TabletQuiz.coursemodule,
            readingStrategy: this.offline ? CoreSitesReadingStrategy.PREFER_CACHE : CoreSitesReadingStrategy.ONLY_NETWORK,
        });
