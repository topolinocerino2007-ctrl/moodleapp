...
import { ADDON_MOD_TABLETQUIZ_COMPONENT_LEGACY } from '../../constants';
import { CoreModals } from '@services/overlays/modals';
import { CoreAlerts } from '@services/overlays/alerts';
import { Translate } from '@singletons';
import { AddonModTabletTabletQuizQuestionCardComponent } from '../../components/question-card/question-card';
import { AddonModTabletTabletQuizAttemptInfoComponent } from '../../components/attempt-info/attempt-info';
...
/**
 * Page that allows reviewing a tabletquiz attempt.
 */
@Component({
    selector: 'page-addon-mod-tablettabletquiz-review',
...
export default class AddonModTabletTabletQuizReviewPage implements OnInit {

    readonly content = viewChild.required(IonContent);

    protected static readonly QUESTIONS_PER_LOAD_WHEN_SHOW_ALL = 10;

    attempt?: AddonModTabletTabletQuizAttemptWSData; // The attempt being reviewed.
    ...
    navigation: AddonModTabletTabletQuizNavigationQuestion[] = []; // List of questions to navigate them.
    ...
    protected options?: AddonModTabletTabletQuizCombinedReviewOptions; // Review options.
    ...
    protected async fetchData(): Promise<void> {
        try {
            ...
            this.options = await AddonModTabletTabletQuiz.getCombinedReviewOptions(this.tabletquiz.id, { cmId: this.cmId });

            AddonModTabletTabletQuizHelper.setTabletQuizCalculatedData(this.tabletquiz, this.options);
    ...
    protected async loadPage(page: number): Promise<void> {
        ...
        const data = await AddonModTabletTabletQuiz.getAttemptReview(this.attemptId, { page, cmId: this.tabletquiz.coursemodule });

        this.attempt = await AddonModTabletTabletQuizHelper.setAttemptCalculatedData(this.tabletquiz, data.attempt);
        ...
        if (page === -1) {
            ...
            this.questions = data.questions.slice(0, AddonModTabletTabletQuizReviewPage.QUESTIONS_PER_LOAD_WHEN_SHOW_ALL);
    ...
    loadMore(infiniteComplete?: () => void): void {
        this.questions = this.questions.concat(this.allQuestions.slice(
            ...
            this.questions.length + AddonModTabletTabletQuizReviewPage.QUESTIONS_PER_LOAD_WHEN_SHOW_ALL,
        ));
        this.canLoadMore = this.allQuestions.length > this.questions.length;
    ...
    protected async loadNavigation(): Promise<void> {
        ...
        const data = await AddonModTabletTabletQuiz.getAttemptReview(this.attemptId, { page: -1, cmId: this.tabletquiz?.coursemodule });
    ...
    async refreshData(refresher: HTMLIonRefresherElement): Promise<void> {
        ...
        promises.push(AddonModTabletTabletQuiz.invalidateAttemptReview(this.attemptId));
        if (this.tabletquiz) {
    ...
    protected async scrollToQuestion(slot: number): Promise<void> {
        if (this.showAll && this.canLoadMore) {
            ...
            if (pageIndex > this.questions.length) {
                ...
                const lastQuestionToLoad = Math.ceil(pageIndex / AddonModTabletTabletQuizReviewPage.QUESTIONS_PER_LOAD_WHEN_SHOW_ALL) *
                    AddonModTabletTabletQuizReviewPage.QUESTIONS_PER_LOAD_WHEN_SHOW_ALL;
    ...
    async openNavigation(): Promise<void> {
        ...
        const modalData = await CoreModals.openSideModal<AddonModTabletTabletQuizNavigationModalReturn>({
            ...
            componentProps: {
                ...
                isReview: true,
                isSequential: this.tabletquiz && AddonModTabletTabletQuiz.isNavigationSequential(this.tabletquiz),
            },
        });
    ...
    protected async performLogView(logInLMS = false, options: LogViewOptions = {}): Promise<void> {
        ...
        if (logInLMS) {
            await CorePromiseUtils.ignoreErrors(AddonModTabletTabletQuiz.logViewAttemptReview(this.attemptId, this.tabletquiz.id));
        }

        let url = `/mod/tabletquiz/review.php?attempt=${this.attemptId}&cmid=${this.cmId}`;
        ...
        CoreAnalytics.logEvent({
            ...
            ws: 'mod_tablettabletquiz_view_attempt_review',
            name: this.tabletquiz.name,
            data: { id: this.attemptId, tabletquizid: this.tabletquiz.id, page: options.page, category: 'tabletquiz' },
            url: url,
        });
    }
