...
export default class AddonModTabletQuizReviewPage implements OnInit {
    ...
    protected async fetchData(): Promise<void> {
        try {
            ...
            this.options = await AddonModTabletQuiz.getCombinedReviewOptions(this.quiz.id, { cmId: this.cmId });

            AddonModTabletQuizHelper.setQuizCalculatedData(this.quiz, this.options);
    ...
    protected async loadPage(page: number): Promise<void> {
        ...
        const data = await AddonModTabletQuiz.getAttemptReview(this.attemptId, { page, cmId: this.quiz.coursemodule });

        this.attempt = await AddonModTabletQuizHelper.setAttemptCalculatedData(this.quiz, data.attempt);
    ...
    protected async loadNavigation(): Promise<void> {
        ...
        const data = await AddonModTabletQuiz.getAttemptReview(this.attemptId, { page: -1, cmId: this.quiz?.coursemodule });
    ...
    async openNavigation(): Promise<void> {
        ...
        const modalData = await CoreModals.openSideModal<AddonModTabletQuizNavigationModalReturn>({
            ...
            componentProps: {
                ...
                isReview: true,
                isSequential: this.quiz && AddonModTabletQuiz.isNavigationSequential(this.quiz),
            },
        });
    ...
    protected async performLogView(logInLMS = false, options: LogViewOptions = {}): Promise<void> {
        ...
        if (logInLMS) {
            await CorePromiseUtils.ignoreErrors(AddonModTabletQuiz.logViewAttemptReview(this.attemptId, this.quiz.id));
        }
