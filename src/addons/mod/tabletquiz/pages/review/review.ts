...
export default class AddonModQuizReviewPage implements OnInit {
    ...
    protected async fetchData(): Promise<void> {
        try {
            ...
            this.options = await AddonModQuiz.getCombinedReviewOptions(this.TabletQuiz.id, { cmId: this.cmId });

            AddonModQuizHelper.setQuizCalculatedData(this.TabletQuiz, this.options);
    ...
    protected async loadPage(page: number): Promise<void> {
        ...
        const data = await AddonModQuiz.getAttemptReview(this.attemptId, { page, cmId: this.TabletQuiz.coursemodule });

        this.attempt = await AddonModQuizHelper.setAttemptCalculatedData(this.TabletQuiz, data.attempt);
    ...
    protected async loadNavigation(): Promise<void> {
        ...
        const data = await AddonModQuiz.getAttemptReview(this.attemptId, { page: -1, cmId: this.TabletQuiz?.coursemodule });
    ...
    async openNavigation(): Promise<void> {
        ...
        const modalData = await CoreModals.openSideModal<AddonModQuizNavigationModalReturn>({
            ...
            componentProps: {
                ...
                isReview: true,
                isSequential: this.TabletQuiz && AddonModQuiz.isNavigationSequential(this.TabletQuiz),
            },
        });
    ...
    protected async performLogView(logInLMS = false, options: LogViewOptions = {}): Promise<void> {
        ...
        if (logInLMS) {
            await CorePromiseUtils.ignoreErrors(AddonModQuiz.logViewAttemptReview(this.attemptId, this.TabletQuiz.id));
        }
