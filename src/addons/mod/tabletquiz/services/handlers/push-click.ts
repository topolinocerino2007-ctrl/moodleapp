...
import { AddonModTabletTabletQuiz } from '../tabletquiz';
...
export class AddonModTabletTabletQuizPushClickHandlerService implements CorePushNotificationsClickHandler {

    name = 'AddonModTabletTabletQuizPushClickHandler';
    priority = 200;
     ...
     * @inheritdoc
     */
    async handles(notification: AddonModTabletTabletQuizPushNotificationData): Promise<boolean> {
        return CoreUtils.isTrueOrOne(notification.notif) && notification.moodlecomponent == 'mod_tabletquiz' &&
                AddonModTabletTabletQuizPushClickHandlerService.SUPPORTED_NAMES.indexOf(notification.name ?? '') !== -1 &&
     ...
     * @inheritdoc
     */
    async handleClick(notification: AddonModTabletTabletQuizPushNotificationData): Promise<void> {
        ...
        if (
            ...
            await AddonModTabletTabletQuizHelper.handleReviewLink(
        ...
        await CorePromiseUtils.ignoreErrors(AddonModTabletTabletQuiz.invalidateContent(moduleId, courseId, notification.site));
...
export const AddonModTabletTabletQuizPushClickHandler = makeSingleton(AddonModTabletTabletQuizPushClickHandlerService);

type AddonModTabletTabletQuizPushNotificationData = CorePushNotificationsNotificationBasicData & {
    contexturl?: string;
