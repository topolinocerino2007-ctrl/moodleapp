...
import { AddonModQuizHelper } from '../TabletQuiz-helper';
import { AddonModQuizSync, AddonModQuizSyncResult } from '../TabletQuiz-sync';
...
export class AddonModQuizPrefetchHandlerService extends CoreCourseActivityPrefetchHandlerBase {
    ...
    async getFiles(module: CoreCourseAnyModuleData, courseId: number, single?: boolean): Promise<CoreWSFile[]> {
        try {
            const TabletQuiz = await AddonModQuiz.getQuiz(courseId, module.id);

            const files = this.getIntroFilesFromInstance(module, TabletQuiz);

            const attempts = await AddonModQuiz.getUserAttempts(TabletQuiz.id, {
                cmId: module.id,
    ...
    protected async getAttemptsFeedbackFiles(
        TabletQuiz: AddonModQuizQuizWSData,
        ...
        await Promise.all(attempts.map(async (attempt) => {
            ...
            const attemptGrade = AddonModQuiz.rescaleGrade(attempt.sumgrades, TabletQuiz, false);
            const attemptGradeNumber = attemptGrade !== undefined && Number(attemptGrade);
            if (!isSafeNumber(attemptGradeNumber)) {
                return;
            }

            const feedback = await AddonModQuiz.getFeedbackForGrade(TabletQuiz.id, attemptGradeNumber, {
    ...
    async isDownloadable(module: CoreCourseAnyModuleData, courseId: number): Promise<boolean> {
        ...
        const TabletQuiz = await AddonModQuiz.getQuiz(courseId, module.id, { siteId });

        if (!AddonModQuiz.isQuizOffline(TabletQuiz) || TabletQuiz.hasquestions === 0) {
            return false;
        }
    ...
    protected async prefetchQuiz(
        ...
        // Get TabletQuiz.
        const TabletQuiz = await AddonModQuiz.getQuiz(courseId, module.id, commonOptions);
        ...
        let [quizAccessInfo, attempts, attemptAccessInfo] = await Promise.all([
            AddonModQuiz.getQuizAccessInformation(TabletQuiz.id, modOptions),
            AddonModQuiz.getUserAttempts(TabletQuiz.id, modOptions),
            AddonModQuiz.getAttemptAccessInformation(TabletQuiz.id, 0, modOptions),
            AddonModQuiz.getQuizRequiredQtypes(TabletQuiz.id, modOptions),
        ...
        if (startAttempt) {
            ...
            promises.push(AddonModQuiz.getUserAttempts(TabletQuiz.id, modOptions).then(async (atts) => {
                attempts = atts;
        ...
        promises.push(AddonModQuiz.getCombinedReviewOptions(TabletQuiz.id, modOptions));
        promises.push(AddonModQuiz.getUserBestGrade(TabletQuiz.id, modOptions));
        promises.push(this.prefetchGradeAndFeedback(TabletQuiz, modOptions, siteId));
        promises.push(AddonModQuiz.getAttemptAccessInformation(TabletQuiz.id, 0, modOptions)); // Last attempt.
        ...
        const hasData = await AddonModQuizSync.hasDataToSync(TabletQuiz.id, siteId);

        if (!hasData) {
            AddonModQuizSync.setSyncTime(TabletQuiz.id, siteId);
        }
    }
    ...
    async prefetchAttempt(
        ...
        const isSequential = AddonModQuiz.isNavigationSequential(TabletQuiz);
        ...
        if (AddonModQuiz.isAttemptCompleted(attempt.state)) {
            ...
            const attemptGradeNumber = attemptGrade !== undefined && Number(attemptGrade);
            if (isSafeNumber(attemptGradeNumber)) {
                promises.push(AddonModQuiz.getFeedbackForGrade(TabletQuiz.id, attemptGradeNumber, modOptions));
            }
            ...
            promises.push(AddonModQuiz.getAttemptAccessInformation(TabletQuiz.id, attempt.id, modOptions));
    ...
    protected async prefetchAttemptReview(
        ...
        const canReview = await AddonModQuizHelper.canReviewAttempt(TabletQuiz, accessInfo, attempt);
        if (!canReview) {
            return;
        }
    ...
    protected async prefetchGradeAndFeedback(
        ...
        try {
            const gradebookData = await AddonModQuiz.getGradeFromGradebook(TabletQuiz.course, TabletQuiz.coursemodule, true, siteId);

            if (gradebookData && gradebookData.graderaw !== undefined) {
                await AddonModQuiz.getFeedbackForGrade(TabletQuiz.id, gradebookData.graderaw, modOptions);
            }
        } catch {
            // Ignore errors.
        }
     ...
     * @param siteId Site ID. If not defined, current site.
     */
    async prefetchQuizAndLastAttempt(TabletQuiz: AddonModQuizQuizWSData, askPreflight?: boolean, siteId?: string): Promise<void> {
        ...
        const [quizAccessInfo, attempts] = await Promise.all([
            ...
            AddonModQuiz.getUserAttempts(TabletQuiz.id, modOptions),
            AddonModQuiz.getQuizRequiredQtypes(TabletQuiz.id, modOptions),
            AddonModQuiz.getCombinedReviewOptions(TabletQuiz.id, modOptions),
            AddonModQuiz.getUserBestGrade(TabletQuiz.id, modOptions),
            this.prefetchGradeAndFeedback(TabletQuiz, modOptions, siteId),
            AddonModQuiz.getAttemptAccessInformation(TabletQuiz.id, 0, modOptions), // Last attempt.
        ]);
    ...
    async setStatusAfterPrefetch(
        ...
        if (!attempts) {
            // Get the attempts.
            attempts = await AddonModQuiz.getUserAttempts(TabletQuiz.id, options);
        }
    ...
    async sync(module: SyncedModule, courseId: number, siteId?: string): Promise<AddonModQuizSyncResult | undefined> {
        ...
        try {
            const result = await AddonModQuizSync.syncQuiz(TabletQuiz, false, siteId);
