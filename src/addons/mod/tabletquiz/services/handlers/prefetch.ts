...
import { AddonModTabletQuizAttemptStates, ADDON_MOD_QUIZ_COMPONENT_LEGACY, ADDON_MOD_QUIZ_MODNAME } from '../../constants';
...
export class AddonModQuizPrefetchHandlerService extends CoreCourseActivityPrefetchHandlerBase {
    ...
    async getFiles(module: CoreCourseAnyModuleData, courseId: number, single?: boolean): Promise<CoreWSFile[]> {
        try {
            const quiz = await AddonModTabletQuiz.getQuiz(courseId, module.id);

            const files = this.getIntroFilesFromInstance(module, quiz);

            const attempts = await AddonModTabletQuiz.getUserAttempts(quiz.id, {
                cmId: module.id,
    ...
    protected async getAttemptsFeedbackFiles(
        ...
        await Promise.all(attempts.map(async (attempt) => {
            ...
            const attemptGrade = AddonModTabletQuiz.rescaleGrade(attempt.sumgrades, quiz, false);
            const attemptGradeNumber = attemptGrade !== undefined && Number(attemptGrade);
            if (!isSafeNumber(attemptGradeNumber)) {
                return;
            }

            const feedback = await AddonModTabletQuiz.getFeedbackForGrade(quiz.id, attemptGradeNumber, {
    ...
    async isDownloadable(module: CoreCourseAnyModuleData, courseId: number): Promise<boolean> {
        ...
        const quiz = await AddonModTabletQuiz.getQuiz(courseId, module.id, { siteId });

        if (!AddonModTabletQuiz.isQuizOffline(quiz) || quiz.hasquestions === 0) {
            return false;
        }
    ...
    protected async prefetchQuiz(
        ...
        // Get quiz.
        const quiz = await AddonModTabletQuiz.getQuiz(courseId, module.id, commonOptions);
        ...
        let [quizAccessInfo, attempts, attemptAccessInfo] = await Promise.all([
            AddonModTabletQuiz.getQuizAccessInformation(quiz.id, modOptions),
            AddonModTabletQuiz.getUserAttempts(quiz.id, modOptions),
            AddonModTabletQuiz.getAttemptAccessInformation(quiz.id, 0, modOptions),
            AddonModTabletQuiz.getQuizRequiredQtypes(quiz.id, modOptions),
        ...
        if (startAttempt) {
            ...
            promises.push(AddonModTabletQuiz.getUserAttempts(quiz.id, modOptions).then(async (atts) => {
                attempts = atts;
        ...
        promises.push(AddonModTabletQuiz.getCombinedReviewOptions(quiz.id, modOptions));
        promises.push(AddonModTabletQuiz.getUserBestGrade(quiz.id, modOptions));
        promises.push(this.prefetchGradeAndFeedback(quiz, modOptions, siteId));
        promises.push(AddonModTabletQuiz.getAttemptAccessInformation(quiz.id, 0, modOptions)); // Last attempt.
        ...
        const hasData = await AddonModTabletQuizSync.hasDataToSync(quiz.id, siteId);

        if (!hasData) {
            AddonModTabletQuizSync.setSyncTime(quiz.id, siteId);
        }
    }
    ...
    async prefetchAttempt(
        ...
        const isSequential = AddonModTabletQuiz.isNavigationSequential(quiz);
        ...
        if (AddonModTabletQuiz.isAttemptCompleted(attempt.state)) {
            ...
            const attemptGradeNumber = attemptGrade !== undefined && Number(attemptGrade);
            if (isSafeNumber(attemptGradeNumber)) {
                promises.push(AddonModTabletQuiz.getFeedbackForGrade(quiz.id, attemptGradeNumber, modOptions));
            }
            ...
            promises.push(AddonModTabletQuiz.getAttemptAccessInformation(quiz.id, attempt.id, modOptions));
    ...
    protected async prefetchAttemptReview(
        ...
        const canReview = await AddonModTabletQuizHelper.canReviewAttempt(quiz, accessInfo, attempt);
        if (!canReview) {
            return;
        }
    ...
    protected async prefetchGradeAndFeedback(
        ...
        try {
            const gradebookData = await AddonModTabletQuiz.getGradeFromGradebook(quiz.course, quiz.coursemodule, true, siteId);

            if (gradebookData && gradebookData.graderaw !== undefined) {
                await AddonModTabletQuiz.getFeedbackForGrade(quiz.id, gradebookData.graderaw, modOptions);
            }
        } catch {
            // Ignore errors.
        }
     ...
     * @param siteId Site ID. If not defined, current site.
     */
    async prefetchQuizAndLastAttempt(quiz: AddonModTabletQuizQuizWSData, askPreflight?: boolean, siteId?: string): Promise<void> {
        ...
        const [quizAccessInfo, attempts] = await Promise.all([
            ...
            AddonModTabletQuiz.getUserAttempts(quiz.id, modOptions),
            AddonModTabletQuiz.getQuizRequiredQtypes(quiz.id, modOptions),
            AddonModTabletQuiz.getCombinedReviewOptions(quiz.id, modOptions),
            AddonModTabletQuiz.getUserBestGrade(quiz.id, modOptions),
            this.prefetchGradeAndFeedback(quiz, modOptions, siteId),
            AddonModTabletQuiz.getAttemptAccessInformation(quiz.id, 0, modOptions), // Last attempt.
        ]);
    ...
    async setStatusAfterPrefetch(
        ...
        if (!attempts) {
            // Get the attempts.
            attempts = await AddonModTabletQuiz.getUserAttempts(quiz.id, options);
        }
    ...
    async sync(module: SyncedModule, courseId: number, siteId?: string): Promise<AddonModTabletQuizSyncResult | undefined> {
        ...
        try {
            const result = await AddonModTabletQuizSync.syncQuiz(quiz, false, siteId);
