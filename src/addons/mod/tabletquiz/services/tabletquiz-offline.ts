...
import { AddonModTabletQuizAttemptDBRecord, ATTEMPTS_TABLE_NAME } from './database/tabletquiz';
import { AddonModTabletQuizAttemptWSData, AddonModTabletQuizTabletQuizWSData } from './tabletquiz';
import { ADDON_MOD_TABLETQUIZ_COMPONENT_LEGACY } from '../constants';
import { CoreQuestionHelper } from '@features/question/services/question-helper';

/**
 * Service to handle offline tabletquiz.
 */
@Injectable({ providedIn: 'root' })
export class AddonModTabletQuizOfflineProvider {

    protected logger: CoreLogger;

    constructor() {
        this.logger = CoreLogger.getInstance('AddonModTabletQuizOfflineProvider');
    }
     ...
     */
    classifyAnswersInQuestions(answers: CoreQuestionsAnswers): AddonModTabletQuizQuestionsWithAnswers {
        const questionsWithAnswers: AddonModTabletQuizQuestionsWithAnswers = {};
     ...
     * @returns Answers.
     */
    extractAnswersFromQuestions(questions: AddonModTabletQuizQuestionsWithAnswers): CoreQuestionsAnswers {
     ...
     */
    async getAllAttempts(siteId?: string): Promise<AddonModTabletQuizAttemptDBRecord[]> {
    ...
    getAttemptAnswers(attemptId: number, siteId?: string): Promise<CoreQuestionAnswerDBRecord[]> {
        return CoreQuestion.getAttemptAnswers(ADDON_MOD_TABLETQUIZ_COMPONENT_LEGACY, attemptId, siteId);
    }
     ...
     */
    async getAttemptById(attemptId: number, siteId?: string): Promise<AddonModTabletQuizAttemptDBRecord> {
        const db = await CoreSites.getSiteDb(siteId);
     ...
     */
    async getTabletQuizAttempts(tabletquizId: number, siteId?: string, userId?: number): Promise<AddonModTabletQuizAttemptDBRecord[]> {
        const site = await CoreSites.getSite(siteId);

        return site.getDb().getRecords(ATTEMPTS_TABLE_NAME, { tabletquizid: tabletquizId, userid: userId || site.getUserId() });
    }
     ...
     * @param questions Object with the questions of the tabletquiz. The keys should be the question slot.
    ...
    async processAttempt(
        ...
        await this.saveAnswers(tabletquiz, attempt, questions, data, now, siteId);
    }
    ...
    async removeAttemptAndAnswers(attemptId: number, siteId?: string): Promise<void> {
        ...
        await Promise.all([
            CoreQuestion.removeAttemptAnswers(ADDON_MOD_TABLETQUIZ_COMPONENT_LEGACY, attemptId, siteId),
            CoreQuestion.removeAttemptQuestions(ADDON_MOD_TABLETQUIZ_COMPONENT_LEGACY, attemptId, siteId),
    ...
    async removeQuestionAndAnswers(attemptId: number, slot: number, siteId?: string): Promise<void> {
        ...
        await Promise.all([
            CoreQuestion.removeQuestion(ADDON_MOD_TABLETQUIZ_COMPONENT_LEGACY, attemptId, slot, siteId),
            CoreQuestion.removeQuestionAnswers(ADDON_MOD_TABLETQUIZ_COMPONENT_LEGACY, attemptId, slot, siteId),
        ]);
    }
    ...
    async saveAnswers(
        ...
        await Promise.all(Object.values(questionsWithAnswers).map(async (question) => {
            ...
            await CoreQuestion.removeQuestionAnswers(ADDON_MOD_TABLETQUIZ_COMPONENT_LEGACY, attempt.id, question.slot, siteId);
        }));
...
export const AddonModTabletQuizOffline = makeSingleton(AddonModTabletQuizOfflineProvider);

/**
 * Answers classified by question slot.
 */
export type AddonModTabletQuizQuestionsWithAnswers = Record<number, {
    prefix: string;
    answers: CoreQuestionsAnswers;
}>;
