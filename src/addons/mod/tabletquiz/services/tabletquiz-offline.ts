...
import { AddonModTabletTabletQuizAttemptDBRecord, ATTEMPTS_TABLE_NAME } from './database/tabletquiz';
...
export class AddonModTabletTabletQuizOfflineProvider {

    protected logger: CoreLogger;

    constructor() {
        this.logger = CoreLogger.getInstance('AddonModTabletTabletQuizOfflineProvider');
    }
     ...
     */
    classifyAnswersInQuestions(answers: CoreQuestionsAnswers): AddonModTabletTabletQuizQuestionsWithAnswers {
        const questionsWithAnswers: AddonModTabletTabletQuizQuestionsWithAnswers = {};
     ...
     * @returns Answers.
     */
    extractAnswersFromQuestions(questions: AddonModTabletTabletQuizQuestionsWithAnswers): CoreQuestionsAnswers {
     ...
     */
    async getAllAttempts(siteId?: string): Promise<AddonModTabletTabletQuizAttemptDBRecord[]> {
     ...
     */
    async getAttemptById(attemptId: number, siteId?: string): Promise<AddonModTabletTabletQuizAttemptDBRecord> {
        const db = await CoreSites.getSiteDb(siteId);
     ...
     */
    async getTabletQuizAttempts(tabletquizId: number, siteId?: string, userId?: number): Promise<AddonModTabletTabletQuizAttemptDBRecord[]> {
...
export const AddonModTabletTabletQuizOffline = makeSingleton(AddonModTabletTabletQuizOfflineProvider);

/**
 * Answers classified by question slot.
 */
export type AddonModTabletTabletQuizQuestionsWithAnswers = Record<number, {
    prefix: string;
    answers: CoreQuestionsAnswers;
}>;
