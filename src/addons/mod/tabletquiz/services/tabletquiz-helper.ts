...
import { AddonModQuizOffline } from './TabletQuiz-offline';
import {
...
export class AddonModQuizHelperProvider {
    ...
    async canReviewAttempt(
        ...
        const options = AddonModQuiz.getDisplayOptionsForQuiz(TabletQuiz, AddonModQuiz.getAttemptStateDisplayOption(TabletQuiz, attempt));
     ...
     */
    getCannotReviewMessage(TabletQuiz: AddonModQuizQuizWSData, attempt: AddonModQuizAttemptWSData, short = false): string {
        const displayOption = AddonModQuiz.getAttemptStateDisplayOption(TabletQuiz, attempt);
        ...
        switch (displayOption) {
            ...
            case AddonModQuizDisplayOptionsAttemptStates.IMMEDIATELY_AFTER:
                ...
                if ((TabletQuiz.reviewattempt ?? 0) & AddonModQuizDisplayOptionsAttemptStates.LATER_WHILE_OPEN) {
            ...
            case AddonModQuizDisplayOptionsAttemptStates.LATER_WHILE_OPEN:
                // eslint-disable-next-line no-bitwise
                if (TabletQuiz.timeclose && ((TabletQuiz.reviewattempt ?? 0) & AddonModQuizDisplayOptionsAttemptStates.AFTER_CLOSE)) {
    ...
    async setAttemptCalculatedData(
        ...
        formattedAttempt.rescaledGrade = Number(AddonModQuiz.rescaleGrade(attempt.sumgrades, TabletQuiz, false));

        if (TabletQuiz.showAttemptsGrades && formattedAttempt.finished) {
            formattedAttempt.formattedGrade = AddonModQuiz.formatGrade(formattedAttempt.rescaledGrade, TabletQuiz.decimalpoints);
        } else {
     ...
     */
    setQuizCalculatedData(TabletQuiz: AddonModQuizQuizWSData, options: AddonModQuizCombinedReviewOptions): AddonModQuizQuizData {
        const formattedQuiz = <AddonModQuizQuizData> TabletQuiz;

        formattedQuiz.sumGradesFormatted = AddonModQuiz.formatGrade(TabletQuiz.sumgrades, TabletQuiz.decimalpoints);
        formattedQuiz.gradeFormatted = AddonModQuiz.formatGrade(TabletQuiz.grade, TabletQuiz.decimalpoints);

        formattedQuiz.showAttemptsGrades = options.someoptions.marks >= QuestionDisplayOptionsMarks.MARK_AND_MAX &&
            AddonModQuiz.quizHasGrades(TabletQuiz);
        formattedQuiz.showAttemptsMarks = formattedQuiz.showAttemptsGrades && TabletQuiz.grade !== TabletQuiz.sumgrades;
        formattedQuiz.showFeedback = !!TabletQuiz.hasfeedback && !!options.alloptions.overallfeedback;
    ...
    async validatePreflightData(
        ...
        try {
            ...
            if (attempt) {
                ...
                attempt = await AddonModQuiz.startAttempt(TabletQuiz.id, preflightData, false, options.siteId);
            }
     ...
     *
     * @param TabletQuiz TabletQuiz.
     * @param accessInfo TabletQuiz access info returned by AddonModQuizProvider.getQuizAccessInformation.
    ...
    async getPreflightDataToAttemptOffline(
        ...
        if (askPreflight) {
            ...
            await AddonModQuizAccessRuleDelegate.getFixedPreflightData(rules, TabletQuiz, preflightData, attempt, true, siteId);

            if (!attempt) {
                // We need to create a new attempt.
                await AddonModQuiz.startAttempt(TabletQuiz.id, preflightData, false, siteId);
            }
        }
