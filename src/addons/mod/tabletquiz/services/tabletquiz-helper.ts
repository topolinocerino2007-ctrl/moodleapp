...
export class AddonModTabletQuizHelperProvider {
    ...
    async canReviewAttempt(
        ...
        const options = AddonModTabletQuiz.getDisplayOptionsForQuiz(quiz, AddonModTabletQuiz.getAttemptStateDisplayOption(quiz, attempt));
     ...
     */
    getCannotReviewMessage(quiz: AddonModTabletQuizQuizWSData, attempt: AddonModTabletQuizAttemptWSData, short = false): string {
        const displayOption = AddonModTabletQuiz.getAttemptStateDisplayOption(quiz, attempt);
        ...
        switch (displayOption) {
            ...
            case AddonModTabletQuizDisplayOptionsAttemptStates.IMMEDIATELY_AFTER:
                ...
                if ((quiz.reviewattempt ?? 0) & AddonModTabletQuizDisplayOptionsAttemptStates.LATER_WHILE_OPEN) {
            ...
            case AddonModTabletQuizDisplayOptionsAttemptStates.LATER_WHILE_OPEN:
                // eslint-disable-next-line no-bitwise
                if (quiz.timeclose && ((quiz.reviewattempt ?? 0) & AddonModTabletQuizDisplayOptionsAttemptStates.AFTER_CLOSE)) {
    ...
    async setAttemptCalculatedData(
        quiz: AddonModTabletQuizQuizData,
        attempt: AddonModTabletQuizAttemptWSData,
        siteId?: string,
        ...
        formattedAttempt.rescaledGrade = Number(AddonModTabletQuiz.rescaleGrade(attempt.sumgrades, quiz, false));

        if (quiz.showAttemptsGrades && formattedAttempt.finished) {
            formattedAttempt.formattedGrade = AddonModTabletQuiz.formatGrade(formattedAttempt.rescaledGrade, quiz.decimalpoints);
        } else {
     ...
     */
    setQuizCalculatedData(quiz: AddonModTabletQuizQuizWSData, options: AddonModTabletQuizCombinedReviewOptions): AddonModTabletQuizQuizData {
        const formattedQuiz = <AddonModTabletQuizQuizData> quiz;

        formattedQuiz.sumGradesFormatted = AddonModTabletQuiz.formatGrade(quiz.sumgrades, quiz.decimalpoints);
        formattedQuiz.gradeFormatted = AddonModTabletQuiz.formatGrade(quiz.grade, quiz.decimalpoints);

        formattedQuiz.showAttemptsGrades = options.someoptions.marks >= QuestionDisplayOptionsMarks.MARK_AND_MAX &&
            AddonModTabletQuiz.quizHasGrades(quiz);
    ...
    async validatePreflightData(
        ...
        try {
            ...
            if (attempt) {
                ...
                attempt = await AddonModTabletQuiz.startAttempt(quiz.id, preflightData, false, options.siteId);
            }
     ...
     *
     * @param quiz Quiz.
     * @param accessInfo Quiz access info returned by AddonModTabletQuizProvider.getQuizAccessInformation.
    ...
    async getPreflightDataToAttemptOffline(
        ...
        if (askPreflight) {
            ...
            await AddonModTabletQuizAccessRuleDelegate.getFixedPreflightData(rules, quiz, preflightData, attempt, true, siteId);

            if (!attempt) {
                // We need to create a new attempt.
                await AddonModTabletQuiz.startAttempt(quiz.id, preflightData, false, siteId);
            }
        }
 ...
 */
export type AddonModTabletQuizQuizData = AddonModTabletQuizQuizWSData & {
