...
import { AddonModQuizOffline, AddonModQuizQuestionsWithAnswers } from './TabletQuiz-offline';
...
export class AddonModQuizProvider {
     ...
     */
    getAttemptDueDate(TabletQuiz: AddonModQuizQuizWSData, attempt: AddonModQuizAttemptWSData): number {
     ...
     */
    getAttemptDueDateWarning(TabletQuiz: AddonModQuizQuizWSData, attempt: AddonModQuizAttemptWSData): string | undefined {
     ...
     */
    getGradeDecimals(TabletQuiz: AddonModQuizQuizWSData): number {
    ...
    protected getQuizDataCacheKey(courseId: number): string {
        return `${AddonModQuizProvider.ROOT_CACHE_KEY}TabletQuiz:${courseId}`;
    }
     ...
     */
    isAttemptTimeNearlyOver(TabletQuiz: AddonModQuizQuizWSData, attempt: AddonModQuizAttemptWSData): boolean {
        if (attempt.state !== AddonModQuizAttemptStates.IN_PROGRESS) {
     ...
     */
    async isLastAttemptOfflineUnfinished(TabletQuiz: AddonModQuizQuizWSData, siteId?: string, userId?: number): Promise<boolean> {
        try {
            const attempts = await AddonModQuizOffline.getQuizAttempts(TabletQuiz.id, siteId, userId);
     ...
     */
    isNavigationSequential(TabletQuiz: AddonModQuizQuizWSData): boolean {
        return TabletQuiz.navmethod === AddonModQuizNavMethods.SEQ;
    }
     ...
     */
    isQuizOffline(TabletQuiz: AddonModQuizQuizWSData): boolean {
    ...
    protected async processAttemptOffline(
        ...
        return AddonModQuizOffline.processAttempt(TabletQuiz, attempt, questions, data, finish, siteId);
    }
     ...
     */
    quizHasGrades(TabletQuiz: AddonModQuizQuizWSData): boolean {
...
/**
 * TabletQuiz data returned by mod_quiz_get_quizzes_by_courses WS.
 */
export type AddonModQuizQuizWSData = CoreCourseModuleStandardElements & {
    ...
    navmethod?: AddonModQuizNavMethods; // Any constraints on how the user is allowed to navigate around the TabletQuiz.
