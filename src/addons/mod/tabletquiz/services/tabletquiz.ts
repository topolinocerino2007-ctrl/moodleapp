...
import { AddonModTabletQuizOffline, AddonModTabletQuizQuestionsWithAnswers } from './tabletquiz-offline';
...
export class AddonModTabletQuizProvider {
    ...
    protected getAttemptAccessInformationCommonCacheKey(tabletquizId: number): string {
        return `${AddonModTabletQuizProvider.ROOT_CACHE_KEY}attemptAccessInformation:${tabletquizId}`;
    }
     ...
     */
    getAttemptDueDate(tabletquiz: AddonModTabletQuizTabletQuizWSData, attempt: AddonModTabletQuizAttemptWSData): number {
        ...
        switch (attempt.state) {
            case AddonModTabletQuizAttemptStates.IN_PROGRESS:
                return dueDate * 1000;

            case AddonModTabletQuizAttemptStates.OVERDUE:
     ...
     */
    getAttemptDueDateWarning(tabletquiz: AddonModTabletQuizTabletQuizWSData, attempt: AddonModTabletQuizAttemptWSData): string | undefined {
        const dueDate = this.getAttemptDueDate(tabletquiz, attempt);

        if (attempt.state === AddonModTabletQuizAttemptStates.OVERDUE) {
    ...
    getAttemptReadableStateName(state: string, finishedOffline = false): string {
        ...
        switch (state) {
            ...
            case AddonModTabletQuizAttemptStates.FINISHED:
                return Translate.instant('addon.mod_tabletquiz.statefinished');

            case AddonModTabletQuizAttemptStates.ABANDONED:
    ...
    async getAttemptSummary(
        ...
        if (options.loadLocal) {
            await AddonModTabletQuizOffline.loadQuestionsLocalStates(attemptId, questions, site.getId());
        }
    ...
    protected getCombinedReviewOptionsCommonCacheKey(tabletquizId: number): string {
        return `${AddonModTabletQuizProvider.ROOT_CACHE_KEY}combinedReviewOptions:${tabletquizId}`;
    }
    ...
    protected getFeedbackForGradeCommonCacheKey(tabletquizId: number): string {
        return `${AddonModTabletQuizProvider.ROOT_CACHE_KEY}feedbackForGrade:${tabletquizId}`;
    }
    ...
    async getFeedbackForGrade(
        ...
        grade: SafeNumber,
        options: CoreCourseCommonModWSOptions = {},
    ): Promise<AddonModTabletQuizGetTabletQuizFeedbackForGradeWSResponse> {
        const site = await CoreSites.getSite(options.siteId);

        const params: AddonModTabletQuizGetTabletQuizFeedbackForGradeWSParams = {
            tabletquizid: tabletquizId,
            grade: grade,
        };
        const preSets: CoreSiteWSPreSets = {
        ...
        return site.read('mod_tabletquiz_get_tabletquiz_feedback_for_grade', params, preSets);
    }
     ...
     */
    getGradeDecimals(tabletquiz: AddonModTabletQuizTabletQuizWSData): number {
    ...
    protected async getTabletQuizByField(
    ...
    ): Promise<AddonModTabletQuizTabletQuizWSData> {

        const site = await CoreSites.getSite(options.siteId);

        const params: AddonModTabletQuizGetTabletQuizzesByCoursesWSParams = {
            courseids: [courseId],
        };
        const preSets: CoreSiteWSPreSets = {
        ...
        const response = await site.read<AddonModTabletQuizGetTabletQuizzesByCoursesWSResponse>(
            'mod_tabletquiz_get_tabletquizzes_by_courses',
            params,
            preSets,
        );
     ...
     * @returns Promise resolved when the tabletquiz is retrieved.
     */
    getTabletQuiz(courseId: number, cmId: number, options: CoreSitesCommonWSOptions = {}): Promise<AddonModTabletQuizTabletQuizWSData> {
     ...
     * @returns Promise resolved when the tabletquiz is retrieved.
     */
    getTabletQuizById(courseId: number, id: number, options: CoreSitesCommonWSOptions = {}): Promise<AddonModTabletQuizTabletQuizWSData> {
    ...
    protected getTabletQuizAccessInformationCacheKey(tabletquizId: number): string {
        return `${AddonModTabletQuizProvider.ROOT_CACHE_KEY}tabletquizAccessInformation:${tabletquizId}`;
    }
    ...
    async getTabletQuizAccessInformation(
        tabletquizId: number,
        options: CoreCourseCommonModWSOptions = {},
    ): Promise<AddonModTabletQuizGetTabletQuizAccessInformationWSResponse> {
        const site = await CoreSites.getSite(options.siteId);

        const params: AddonModTabletQuizGetTabletQuizAccessInformationWSParams = {
            tabletquizid: tabletquizId,
        };
        const preSets: CoreSiteWSPreSets = {
        ...
        return site.read('mod_tabletquiz_get_tabletquiz_access_information', params, preSets);
    }
    ...
    protected getTabletQuizRequiredQtypesCacheKey(tabletquizId: number): string {
        return `${AddonModTabletQuizProvider.ROOT_CACHE_KEY}tabletquizRequiredQtypes:${tabletquizId}`;
    }
    ...
    async getTabletQuizRequiredQtypes(tabletquizId: number, options: CoreCourseCommonModWSOptions = {}): Promise<string[]> {
        ...
        const params: AddonModTabletQuizGetTabletQuizRequiredQtypesWSParams = {
            tabletquizid: tabletquizId,
        };
        const preSets: CoreSiteWSPreSets = {
        ...
        const response = await site.read<AddonModTabletQuizGetTabletQuizRequiredQtypesWSResponse>(
            'mod_tabletquiz_get_tabletquiz_required_qtypes',
            params,
            preSets,
        );
     ...
     * This function returns a list of pages.
     */
    getPagesFromLayoutAndQuestions(layout: string, questions: AddonModTabletQuizQuestionsWithAnswers): number[] {
        const split = layout.split(',');
        const pages: number[] = [];
        let page = 0;
        let pageAdded = false;
    ...
    getUnsupportedRules(rulesNames: string[]): string[] {
        ...
        rulesNames.forEach((name) => {
            if (!AddonModTabletQuizAccessRuleDelegate.isAccessRuleSupported(name)) {
    ...
    protected getUserAttemptsCommonCacheKey(tabletquizId: number): string {
        return `${AddonModTabletQuizProvider.ROOT_CACHE_KEY}userAttempts:${tabletquizId}`;
    }
    ...
    protected getUserBestGradeCommonCacheKey(tabletquizId: number): string {
        return `${AddonModTabletQuizProvider.ROOT_CACHE_KEY}userBestGrade:${tabletquizId}`;
    }
     ...
     */
    async getUserBestGrade(tabletquizId: number, options: AddonModTabletQuizUserOptions = {}): Promise<AddonModTabletQuizGetUserBestGradeWSResponse> {
        const site = await CoreSites.getSite(options.siteId);
    ...
    isAttemptCompleted(state?: string): boolean {
        return state === AddonModTabletQuizAttemptStates.FINISHED || state === AddonModTabletQuizAttemptStates.ABANDONED;
    }
    ...
    async isAttemptFinishedOffline(attemptId: number, siteId?: string): Promise<boolean> {
        try {
            const attempt = await AddonModTabletQuizOffline.getAttemptById(attemptId, siteId);
     ...
     */
    isAttemptTimeNearlyOver(tabletquiz: AddonModTabletQuizTabletQuizWSData, attempt: AddonModTabletQuizAttemptWSData): boolean {
        if (attempt.state !== AddonModTabletQuizAttemptStates.IN_PROGRESS) {
     ...
     */
    async isLastAttemptOfflineUnfinished(tabletquiz: AddonModTabletQuizTabletQuizWSData, siteId?: string, userId?: number): Promise<boolean> {
        try {
            const attempts = await AddonModTabletQuizOffline.getTabletQuizAttempts(tabletquiz.id, siteId, userId);
     ...
     */
    isNavigationSequential(tabletquiz: AddonModTabletQuizTabletQuizWSData): boolean {
        return tabletquiz.navmethod === AddonModTabletQuizNavMethods.SEQ;
    }
     ...
     */
    isTabletQuizOffline(tabletquiz: AddonModTabletQuizTabletQuizWSData): boolean {
    ...
    async logViewAttempt(
        ...
        if (offline) {
            promises.push(AddonModTabletQuizOffline.setAttemptCurrentPage(attemptId, page, site.getId()));
        }
    ...
    logViewTabletQuiz(id: number, siteId?: string): Promise<void> {
        const params: AddonModTabletQuizViewTabletQuizWSParams = {
            tabletquizid: id,
        };

        return CoreCourseLogHelper.log(
            'mod_tabletquiz_view_tabletquiz',
            params,
            ADDON_MOD_TABLETQUIZ_COMPONENT_LEGACY,
            id,
            siteId,
    ...
    protected async processAttemptOffline(
        ...
        return AddonModTabletQuizOffline.processAttempt(tabletquiz, attempt, questions, data, finish, siteId);
    }
     ...
     */
    tabletquizHasGrades(tabletquiz: AddonModTabletQuizTabletQuizWSData): boolean {
    ...
    shouldShowTimeLeft(rules: string[], attempt: AddonModTabletQuizAttemptWSData, endTime: number): boolean {
        ...
        return AddonModTabletQuizAccessRuleDelegate.shouldShowTimeLeft(rules, attempt, endTime, timeNow);
    }
...
/**
 * Params of mod_tabletquiz_get_tabletquiz_feedback_for_grade WS.
 */
export type AddonModTabletQuizGetTabletQuizFeedbackForGradeWSParams = {
    tabletquizid: number; // TabletQuiz instance id.
    grade: number; // The grade to check.
};

/**
 * Data returned by mod_tabletquiz_get_tabletquiz_feedback_for_grade WS.
 */
export type AddonModTabletQuizGetTabletQuizFeedbackForGradeWSResponse = {
...
/**
 * Params of mod_tabletquiz_get_tabletquizzes_by_courses WS.
 */
export type AddonModTabletQuizGetTabletQuizzesByCoursesWSParams = {
    courseids?: number[]; // Array of course ids.
};

/**
 * Data returned by mod_tabletquiz_get_tabletquizzes_by_courses WS.
 */
export type AddonModTabletQuizGetTabletQuizzesByCoursesWSResponse = {
    tabletquizzes: AddonModTabletQuizTabletQuizWSData[];
    warnings?: CoreWSExternalWarning[];
};

/**
 * TabletQuiz data returned by mod_tabletquiz_get_tabletquizzes_by_courses WS.
 */
export type AddonModTabletQuizTabletQuizWSData = CoreCourseModuleStandardElements & {
    ...
    navmethod?: AddonModTabletQuizNavMethods; // Any constraints on how the user is allowed to navigate around the tabletquiz.
...
/**
 * Params of mod_tabletquiz_get_tabletquiz_access_information WS.
 */
export type AddonModTabletQuizGetTabletQuizAccessInformationWSParams = {
    tabletquizid: number; // TabletQuiz instance id.
};

/**
 * Data returned by mod_tabletquiz_get_tabletquiz_access_information WS.
 */
export type AddonModTabletQuizGetTabletQuizAccessInformationWSResponse = {
...
/**
 * Params of mod_tabletquiz_get_tabletquiz_required_qtypes WS.
 */
export type AddonModTabletQuizGetTabletQuizRequiredQtypesWSParams = {
    tabletquizid: number; // TabletQuiz instance id.
};

/**
 * Data returned by mod_tabletquiz_get_tabletquiz_required_qtypes WS.
 */
export type AddonModTabletQuizGetTabletQuizRequiredQtypesWSResponse = {
...
/**
 * Params of mod_tabletquiz_view_tabletquiz WS.
 */
export type AddonModTabletQuizViewTabletQuizWSParams = {
    tabletquizid: number; // TabletQuiz instance id.
};
