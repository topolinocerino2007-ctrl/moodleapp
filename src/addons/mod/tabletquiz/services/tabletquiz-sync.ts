...
import { AddonModTabletQuiz, AddonModTabletQuizAttemptWSData, AddonModTabletQuizTabletQuizWSData } from './tabletquiz';
...
export class AddonModTabletQuizSyncProvider extends CoreCourseActivitySyncBaseProvider<AddonModTabletQuizSyncResult> {
    ...
    protected async finishSync(
        ...
        await CorePromiseUtils.ignoreErrors(
            AddonModTabletQuiz.invalidateAllTabletQuizData(tabletquiz.id, courseId, options.attemptId, siteId),
        );
    ...
    async hasDataToSync(tabletquizId: number, siteId?: string): Promise<boolean> {
        try {
            const attempts = await AddonModTabletQuizOffline.getTabletQuizAttempts(tabletquizId, siteId);
    ...
    protected async syncAllTabletQuizzesFunc(force: boolean, siteId: string): Promise<void> {
        ...
        await Promise.all(attempts.map(async (attempt) => {
            ...
            const tabletquiz = await AddonModTabletQuiz.getTabletQuizById(attempt.courseid, attempt.tabletquizid, { siteId });
     ...
     */
    syncTabletQuiz(tabletquiz: AddonModTabletQuizTabletQuizWSData, askPreflight?: boolean, siteId?: string): Promise<AddonModTabletQuizSyncResult> {
     ...
     * @returns Promise resolved in success.
     */
    async performSyncTabletQuiz(tabletquiz: AddonModTabletQuizTabletQuizWSData, askPreflight?: boolean, siteId?: string): Promise<AddonModTabletQuizSyncResult> {
        siteId = siteId || CoreSites.getCurrentSiteId();
        ...
        const offlineAttempts = await AddonModTabletQuizOffline.getTabletQuizAttempts(tabletquiz.id, siteId);
        ...
        const info = await AddonModTabletQuiz.getTabletQuizAccessInformation(tabletquiz.id, modOptions);
