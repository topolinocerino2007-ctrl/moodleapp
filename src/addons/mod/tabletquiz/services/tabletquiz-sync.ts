...
import { AddonModTabletQuizAttemptDBRecord } from './database/tabletquiz';
import { type AddonModTabletQuizPrefetchHandlerService } from './handlers/prefetch';
import { AddonModTabletQuiz, AddonModTabletQuizAttemptWSData, AddonModTabletQuizQuizWSData } from './tabletquiz';
...
export class AddonModTabletQuizSyncProvider extends CoreCourseActivitySyncBaseProvider<AddonModTabletQuizSyncResult> {

    protected componentTranslatableString = 'tabletquiz';

    constructor() {
        super('AddonModTabletQuizSyncProvider');
    }
    ...
    protected async finishSync(
    ...
    ): Promise<AddonModTabletQuizSyncResult> {
        options = options || {};

        // Invalidate the data for the TabletQuiz and attempt.
        await CorePromiseUtils.ignoreErrors(
            AddonModTabletQuiz.invalidateAllQuizData(TabletQuiz.id, courseId, options.attemptId, siteId),
        );

        if (options.removeAttempt && options.attemptId) {
            const promises: Promise<unknown>[] = [];

            promises.push(AddonModTabletQuizOffline.removeAttemptAndAnswers(options.attemptId, siteId));
        ...
        if (options.updated) {
            try {
                ...
                await this.prefetchAfterUpdateQuiz(module, TabletQuiz, courseId, siteId);
            } catch {
                // Ignore errors.
            }
        }
        ...
        if (options.onlineAttempt && !AddonModTabletQuiz.isAttemptCompleted(options.onlineAttempt.state)) {
            // Attempt wasn't finished at start. Check if it's finished now.
            const attempts = await AddonModTabletQuiz.getUserAttempts(TabletQuiz.id, { cmId: TabletQuiz.coursemodule, siteId });

            const attempt = attempts.find(attempt => attempt.id == options?.onlineAttempt?.id);

            attemptFinished = attempt ? AddonModTabletQuiz.isAttemptCompleted(attempt.state) : false;
        }
    ...
    async hasDataToSync(quizId: number, siteId?: string): Promise<boolean> {
        try {
            const attempts = await AddonModTabletQuizOffline.getQuizAttempts(quizId, siteId);
    ...
    protected async prefetchAfterUpdateQuiz(
        ...
        const prefetchHandler =
            CoreCourseModulePrefetchDelegate.getPrefetchHandlerFor<AddonModTabletQuizPrefetchHandlerService>(module.modname);
        if (!prefetchHandler) {
            return;
        }
    ...
    protected async syncAllQuizzesFunc(force: boolean, siteId: string): Promise<void> {
        ...
        const attempts = await AddonModTabletQuizOffline.getAllAttempts(siteId);
        ...
        await Promise.all(attempts.map(async (attempt) => {
            ...
            if (CoreSync.isBlocked(ADDON_MOD_TABLETQUIZ_COMPONENT, attempt.quizid, siteId)) {
                return;
            }

            // TabletQuiz not blocked, try to synchronize it.
            const TabletQuiz = await AddonModTabletQuiz.getQuizById(attempt.courseid, attempt.quizid, { siteId });

            const data = await (force ? this.syncQuiz(TabletQuiz, false, siteId) : this.syncQuizIfNeeded(TabletQuiz, false, siteId));
    ...
    async syncQuizIfNeeded(
        ...
        siteId?: string,
    ): Promise<AddonModTabletQuizSyncResult | undefined> {
        const needed = await this.isSyncNeeded(TabletQuiz.id, siteId);

        if (needed) {
            return this.syncQuiz(TabletQuiz, askPreflight, siteId);
        }
    }
     ...
     */
    syncQuiz(TabletQuiz: AddonModTabletQuizQuizWSData, askPreflight?: boolean, siteId?: string): Promise<AddonModTabletQuizSyncResult> {
        ...
        return this.addOngoingSync(TabletQuiz.id, this.performSyncQuiz(TabletQuiz, askPreflight, siteId), siteId);
    }
     ...
     * @returns Promise resolved in success.
     */
    async performSyncQuiz(TabletQuiz: AddonModTabletQuizQuizWSData, askPreflight?: boolean, siteId?: string): Promise<AddonModTabletQuizSyncResult> {
        siteId = siteId || CoreSites.getCurrentSiteId();
        ...
        const offlineAttempts = await AddonModTabletQuizOffline.getQuizAttempts(TabletQuiz.id, siteId);
        ...
        const onlineAttempts = await AddonModTabletQuiz.getUserAttempts(TabletQuiz.id, modOptions);
        ...
        if (!onlineAttempt || AddonModTabletQuiz.isAttemptCompleted(onlineAttempt.state)) {
            // Attempt not found or it's finished in online. Discard it.
            warnings.push(Translate.instant('addon.mod_tabletquiz.warningattemptfinished'));
        ...
        const answersList = await AddonModTabletQuizOffline.getAttemptAnswers(offlineAttempt.id, siteId);
        ...
        const offlineQuestions = AddonModTabletQuizOffline.classifyAnswersInQuestions(offlineAnswers);

        // We're going to need preflightData, get it.
        const info = await AddonModTabletQuiz.getQuizAccessInformation(TabletQuiz.id, modOptions);

        const preflightData = await AddonModTabletQuizHelper.getPreflightDataToAttemptOffline(
            TabletQuiz,
            info,
            onlineAttempt,
            askPreflight,
        ...
        const onlineQuestions = await AddonModTabletQuiz.getAllQuestionsData(TabletQuiz, onlineAttempt, preflightData, {
            pages: AddonModTabletQuiz.getPagesFromLayoutAndQuestions(onlineAttempt.layout || '', offlineQuestions),
            readingStrategy: CoreSitesReadingStrategy.ONLY_NETWORK,
            siteId,
        });
        ...
        const answers = AddonModTabletQuizOffline.extractAnswersFromQuestions(offlineQuestions);
        const finish = !!offlineAttempt.finished && !discardedData;

        if (discardedData) {
            if (offlineAttempt.finished) {
                warnings.push(Translate.instant('addon.mod_tabletquiz.warningdatadiscardedfromfinished'));
            } else {
                warnings.push(Translate.instant('addon.mod_tabletquiz.warningdatadiscarded'));
            }
        }

        // Send the answers.
        await AddonModTabletQuiz.processAttempt(TabletQuiz, onlineAttempt, answers, preflightData, finish, false, false, siteId);

        if (!finish) {
            // Answers sent, now set the current page.
            await CorePromiseUtils.ignoreErrors(AddonModTabletQuiz.logViewAttempt(
                onlineAttempt.id,
    ...
    async validateQuestions(
        ...
        offlineQuestions: AddonModTabletQuizQuestionsWithAnswers,
        siteId?: string,
    ): Promise<boolean> {
        ...
        await Promise.all(Object.keys(offlineQuestions).map(async (slotString) => {
            ...
            if (onlineQuestion) {
                ...
                if (!CoreQuestionDelegate.validateSequenceCheck(onlineQuestion, offlineSequenceCheck)) {
                    ...
                    await AddonModTabletQuizOffline.removeQuestionAndAnswers(attemptId, slot, siteId);
...
export const AddonModTabletQuizSync = makeSingleton(AddonModTabletQuizSyncProvider);

/**
 * Data returned by a TabletQuiz sync.
 */
export type AddonModTabletQuizSyncResult = CoreSyncResult & {
...
type FinishSyncOptions = {
    ...
    offlineAttempt?: AddonModTabletQuizAttemptDBRecord; // Offline attempt synchronized, if any.
    onlineAttempt?: AddonModTabletQuizAttemptWSData; // Online data for the offline attempt.
 ...
 */
export type AddonModTabletQuizAutoSyncData = {
    quizId: number;
...
declare module '@singletons/events' {
    ...
    export interface CoreEventsData {
        [ADDON_MOD_TABLETQUIZ_AUTO_SYNCED]: AddonModTabletQuizAutoSyncData;
    }
