...
import { AddonModQuiz, AddonModQuizAttemptWSData, AddonModQuizQuizWSData } from './TabletQuiz';
...
export class AddonModQuizSyncProvider extends CoreCourseActivitySyncBaseProvider<AddonModQuizSyncResult> {
    ...
    protected async finishSync(
        ...
        await CorePromiseUtils.ignoreErrors(
            AddonModQuiz.invalidateAllQuizData(TabletQuiz.id, courseId, options.attemptId, siteId),
        );
        ...
        if (options.updated) {
            try {
                ...
                const module = await CoreCourse.getModuleBasicInfoByInstance(TabletQuiz.id, ADDON_MOD_QUIZ_MODNAME, { siteId });
        ...
        if (options.onlineAttempt && !AddonModQuiz.isAttemptCompleted(options.onlineAttempt.state)) {
            ...
            const attempts = await AddonModQuiz.getUserAttempts(TabletQuiz.id, { cmId: TabletQuiz.coursemodule, siteId });
    ...
    protected async syncAllQuizzesFunc(force: boolean, siteId: string): Promise<void> {
        ...
        await Promise.all(attempts.map(async (attempt) => {
            ...
            const TabletQuiz = await AddonModQuiz.getQuizById(attempt.courseid, attempt.quizid, { siteId });
     ...
     */
    syncQuiz(TabletQuiz: AddonModQuizQuizWSData, askPreflight?: boolean, siteId?: string): Promise<AddonModQuizSyncResult> {
        ...
        if (CoreSync.isBlocked(ADDON_MOD_QUIZ_COMPONENT, TabletQuiz.id, siteId)) {
     ...
     * @returns Promise resolved in success.
     */
    async performSyncQuiz(TabletQuiz: AddonModQuizQuizWSData, askPreflight?: boolean, siteId?: string): Promise<AddonModQuizSyncResult> {
        siteId = siteId || CoreSites.getCurrentSiteId();
        ...
        // Sync offline logs.
        await CorePromiseUtils.ignoreErrors(
            CoreCourseLogHelper.syncActivity(ADDON_MOD_QUIZ_COMPONENT_LEGACY, TabletQuiz.id, siteId),
        );

        // Get all the offline attempts for the TabletQuiz. It should always be 0 or 1 attempt
        const offlineAttempts = await AddonModQuizOffline.getQuizAttempts(TabletQuiz.id, siteId);
        ...
        const onlineAttempts = await AddonModQuiz.getUserAttempts(TabletQuiz.id, modOptions);
        ...
        const info = await AddonModQuiz.getQuizAccessInformation(TabletQuiz.id, modOptions);
        ...
        const onlineQuestions = await AddonModQuiz.getAllQuestionsData(TabletQuiz, onlineAttempt, preflightData, {
        ...
        // Send the answers.
        await AddonModQuiz.processAttempt(TabletQuiz, onlineAttempt, answers, preflightData, finish, false, false, siteId);
