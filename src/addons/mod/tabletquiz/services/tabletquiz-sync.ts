...
import { AddonModTabletQuizHelper } from './quiz-helper';
...
export class AddonModTabletQuizSyncProvider extends CoreCourseActivitySyncBaseProvider<AddonModTabletQuizSyncResult> {
    ...
    protected async finishSync(
        ...
        await CorePromiseUtils.ignoreErrors(
            AddonModTabletQuiz.invalidateAllQuizData(quiz.id, courseId, options.attemptId, siteId),
        );
        ...
        if (options.updated) {
            try {
                ...
                const module = await CoreCourse.getModuleBasicInfoByInstance(quiz.id, ADDON_MOD_TABLETQUIZ_MODNAME, { siteId });
        ...
        if (options.onlineAttempt && !AddonModTabletQuiz.isAttemptCompleted(options.onlineAttempt.state)) {
            ...
            const attempts = await AddonModTabletQuiz.getUserAttempts(quiz.id, { cmId: quiz.coursemodule, siteId });
    ...
    protected async syncAllQuizzesFunc(force: boolean, siteId: string): Promise<void> {
        ...
        await Promise.all(attempts.map(async (attempt) => {
            ...
            const quiz = await AddonModTabletQuiz.getQuizById(attempt.courseid, attempt.quizid, { siteId });
     ...
     */
    syncQuiz(quiz: AddonModTabletQuizQuizWSData, askPreflight?: boolean, siteId?: string): Promise<AddonModTabletQuizSyncResult> {
        ...
        if (CoreSync.isBlocked(ADDON_MOD_TABLETQUIZ_COMPONENT, quiz.id, siteId)) {
     ...
     * @returns Promise resolved in success.
     */
    async performSyncQuiz(quiz: AddonModTabletQuizQuizWSData, askPreflight?: boolean, siteId?: string): Promise<AddonModTabletQuizSyncResult> {
        siteId = siteId || CoreSites.getCurrentSiteId();
        ...
        // Sync offline logs.
        await CorePromiseUtils.ignoreErrors(
            CoreCourseLogHelper.syncActivity(ADDON_MOD_TABLETQUIZ_COMPONENT_LEGACY, quiz.id, siteId),
        );

        // Get all the offline attempts for the quiz. It should always be 0 or 1 attempt
        const offlineAttempts = await AddonModTabletQuizOffline.getQuizAttempts(quiz.id, siteId);
        ...
        const onlineAttempts = await AddonModTabletQuiz.getUserAttempts(quiz.id, modOptions);
        ...
        const info = await AddonModTabletQuiz.getQuizAccessInformation(quiz.id, modOptions);
        ...
        const onlineQuestions = await AddonModTabletQuiz.getAllQuestionsData(quiz, onlineAttempt, preflightData, {
        ...
        // Send the answers.
        await AddonModTabletQuiz.processAttempt(quiz, onlineAttempt, answers, preflightData, finish, false, false, siteId);
