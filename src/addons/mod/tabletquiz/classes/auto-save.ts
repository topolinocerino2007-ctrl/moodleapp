...
import { AddonModTabletTabletQuiz, AddonModTabletTabletQuizAttemptWSData, AddonModTabletTabletQuizTabletQuizWSData } from '../services/tabletquiz';
import { CorePopovers } from '@services/overlays/popovers';

/**
 * Class to support auto-save in tabletquiz. Every certain seconds, it will check if there are changes in the current page answers
 * and, if so, it will save them automatically.
 */
export class AddonModTabletTabletQuizAutoSave {
    ...
    constructor(
        ...
        this.logger = CoreLogger.getInstance('AddonModTabletTabletQuizAutoSave');
     ...
     * @param offline Whether the tabletquiz is being attempted in offline mode.
     */
    checkChanges(
        ...
        if (!equal) {
            this.setAutoSaveTimer(tabletquiz, attempt, preflightData, offline);
        }
    ...
    setAutoSaveTimer(
        ...
        // Don't schedule if already shceduled or tabletquiz is almost closed.
        if (!tabletquiz.autosaveperiod || this.autoSaveTimeout || AddonModTabletTabletQuiz.isAttemptTimeNearlyOver(tabletquiz, attempt)) {
            return;
        }
        ...
        this.autoSaveTimeout = window.setTimeout(async () => {
            ...
            try {
                await AddonModTabletTabletQuiz.saveAttempt(tabletquiz, attempt, answers, preflightData, offline);
        ...
        }, tabletquiz.autosaveperiod * 1000);
    }
    ...
    async showAutoSaveError(ev?: Event): Promise<void> {
        ...
        const { AddonModTabletTabletQuizConnectionErrorComponent } = await import('../components/connection-error/connection-error');

        this.popover = await CorePopovers.openWithoutResult({
            component: AddonModTabletTabletQuizConnectionErrorComponent,
            event: <Event> event,
        });
    ...
    startCheckChangesProcess(
        ...
        if (this.checkChangesInterval || !tabletquiz.autosaveperiod) {
            // We already have the interval in place or the tabletquiz has autosave disabled.
            return;
        }
        ...
        this.loadPreviousAnswersTimeout = window.setTimeout(() => {
            this.checkChanges(tabletquiz, attempt, preflightData, offline);
        }, 2500);

        // Check changes every certain time.
        this.checkChangesInterval = window.setInterval(() => {
            this.checkChanges(tabletquiz, attempt, preflightData, offline);
        }, AddonModTabletTabletQuizAutoSave.CHECK_CHANGES_INTERVAL);
    }
