...
import { AddonModTabletQuiz, AddonModTabletQuizAttemptWSData, AddonModTabletQuizQuizWSData } from '../services/quiz';
...
export class AddonModTabletQuizAutoSave {
    ...
    setAutoSaveTimer(
        ...
        if (!quiz.autosaveperiod || this.autoSaveTimeout || AddonModTabletQuiz.isAttemptTimeNearlyOver(quiz, attempt)) {
            return;
        }
        ...
        this.autoSaveTimeout = window.setTimeout(async () => {
            ...
            try {
                await AddonModTabletQuiz.saveAttempt(quiz, attempt, answers, preflightData, offline);
