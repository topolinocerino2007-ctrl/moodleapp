...
import { AddonModQuiz, AddonModQuizAttemptWSData, AddonModQuizQuizWSData } from '../services/TabletQuiz';
...
export class AddonModQuizAutoSave {
    ...
    setAutoSaveTimer(
        ...
        if (!TabletQuiz.autosaveperiod || this.autoSaveTimeout || AddonModQuiz.isAttemptTimeNearlyOver(TabletQuiz, attempt)) {
            return;
        }
        ...
        this.autoSaveTimeout = window.setTimeout(async () => {
            ...
            try {
                await AddonModQuiz.saveAttempt(TabletQuiz, attempt, answers, preflightData, offline);
