...
import { AddonModTabletQuiz, AddonModTabletQuizAttemptWSData, AddonModTabletQuizTabletQuizWSData } from '../services/tabletquiz';
 ...
 */
export class AddonModTabletQuizAutoSave {
    ...
    constructor(
        ...
        this.logger = CoreLogger.getInstance('AddonModTabletQuizAutoSave');
    ...
    setAutoSaveTimer(
        ...
        if (!tabletquiz.autosaveperiod || this.autoSaveTimeout || AddonModTabletQuiz.isAttemptTimeNearlyOver(tabletquiz, attempt)) {
            return;
        }
        ...
        this.autoSaveTimeout = window.setTimeout(async () => {
            ...
            try {
                await AddonModTabletQuiz.saveAttempt(tabletquiz, attempt, answers, preflightData, offline);
    ...
    async showAutoSaveError(ev?: Event): Promise<void> {
        ...
        const { AddonModTabletQuizConnectionErrorComponent } = await import('../components/connection-error/connection-error');

        this.popover = await CorePopovers.openWithoutResult({
            component: AddonModTabletQuizConnectionErrorComponent,
            event: <Event> event,
        });
    ...
    startCheckChangesProcess(
        ...
        this.checkChangesInterval = window.setInterval(() => {
            this.checkChanges(tabletquiz, attempt, preflightData, offline);
        }, AddonModTabletQuizAutoSave.CHECK_CHANGES_INTERVAL);
    }
