...
/**
 * Modal that renders the tabletquiz navigation.
 */
@Component({
    selector: 'addon-mod-tablettabletquiz-navigation-modal',
...
export class AddonModTabletTabletQuizNavigationModalComponent implements OnInit {

    @Input() navigation?: AddonModTabletTabletQuizNavigationQuestion[]; // Whether the user is reviewing the attempt.
    ...
    @Input({ transform: toBoolean }) isSequential = false; // Whether tabletquiz navigation is sequential.
    ...
    loadPage(page: number, slot?: number): void {
        ModalController.dismiss(<AddonModTabletTabletQuizNavigationModalReturn>{
            page,
            slot,
        });
    }
 ...
 */
export type AddonModTabletTabletQuizNavigationQuestion = CoreQuestionQuestionParsed & {
    stateClass?: string;
};

export type AddonModTabletTabletQuizNavigationModalReturn = {
    page: number;
    slot?: number;
};
