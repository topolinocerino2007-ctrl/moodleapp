 ...
 */
@Component({
    selector: 'addon-mod-tabletquiz-navigation-modal',
...
export class AddonModTabletQuizNavigationModalComponent implements OnInit {

    @Input() navigation?: AddonModTabletQuizNavigationQuestion[]; // Whether the user is reviewing the attempt.
    ...
    loadPage(page: number, slot?: number): void {
        ModalController.dismiss(<AddonModTabletQuizNavigationModalReturn>{
            page,
            slot,
        });
    }
 ...
 */
export type AddonModTabletQuizNavigationQuestion = CoreQuestionQuestionParsed & {
    stateClass?: string;
};

export type AddonModTabletQuizNavigationModalReturn = {
    page: number;
    slot?: number;
};
