...
import { AddonModTabletQuizAccessRuleDelegate } from '../../services/access-rules-delegate';
import { AddonModTabletQuizAttemptWSData, AddonModTabletQuizTabletQuizWSData } from '../../services/tabletquiz';
 ...
 */
@Component({
    selector: 'page-addon-mod-tabletquiz-preflight-modal',
    templateUrl: 'preflight-modal.html',
    imports: [
        CoreSharedModule,
    ],
})
export class AddonModTabletQuizPreflightModalComponent implements OnInit {

    readonly formElement = viewChild<ElementRef>('preflightFormEl');

    @Input({ required: true }) title!: string;
    @Input() tabletquiz?: AddonModTabletQuizTabletQuizWSData;
    @Input() attempt?: AddonModTabletQuizAttemptWSData;
    ...
    async ngOnInit(): Promise<void> {
        this.title = this.title || Translate.instant('addon.mod_tabletquiz.startattempt');
        ...
        try {
            ...
            await Promise.all(this.rules.map(async (rule) => {
                ...
                const required = await AddonModTabletQuizAccessRuleDelegate.isPreflightCheckRequiredForRule(
                    rule,
                    tabletquiz,
                    this.attempt,
                    this.prefetch,
                ...
                const component = await AddonModTabletQuizAccessRuleDelegate.getPreflightComponent(rule);
                if (!component) {
                    return;
                }
