...
import { AddonModQuizAttempt, AddonModQuizQuizData } from '../../services/TabletQuiz-helper';
import { AddonModQuiz, AddonModQuizWSAdditionalData } from '../../services/TabletQuiz';
...
export class AddonModQuizAttemptInfoComponent implements OnChanges {
    ...
    @Input({ required: true }) TabletQuiz!: AddonModQuizQuizData;
    ...
    async ngOnChanges(changes: SimpleChanges): Promise<void> {
        ...
        if (this.attempt.sumgrades === null || !this.attempt.gradeitemmarks) {
            ...
            this.gradeItemMarks = this.attempt.gradeitemmarks.map((gradeItemMark) => ({
                ...
                grade: Translate.instant('addon.mod_quiz.outof', { $a: {
                    grade: `<strong>${AddonModQuiz.formatGrade(gradeItemMark.grade, this.TabletQuiz?.decimalpoints)}</strong>`,
                    maxgrade: AddonModQuiz.formatGrade(gradeItemMark.maxgrade, this.TabletQuiz?.decimalpoints),
                } }),
            }));
        }
        ...
        if (this.TabletQuiz.showAttemptsMarks) {
            this.readableMark = Translate.instant('addon.mod_quiz.outofshort', { $a: {
                grade: AddonModQuiz.formatGrade(this.attempt.sumgrades, this.TabletQuiz.decimalpoints),
                maxgrade: AddonModQuiz.formatGrade(this.TabletQuiz.sumgrades, this.TabletQuiz.decimalpoints),
            } });
        }

        const gradeObject: Record<string, unknown> = {
            grade: `<strong>${AddonModQuiz.formatGrade(this.attempt.rescaledGrade, this.TabletQuiz.decimalpoints)}</strong>`,
            maxgrade: AddonModQuiz.formatGrade(this.TabletQuiz.grade, this.TabletQuiz.decimalpoints),
        };

        if (this.TabletQuiz.grade != 100) {
            const percentage = (this.attempt.sumgrades ?? 0) * 100 / (this.TabletQuiz.sumgrades ?? 1);
            gradeObject.percent = `<strong>${AddonModQuiz.formatGrade(percentage, this.TabletQuiz.decimalpoints)}</strong>`;
