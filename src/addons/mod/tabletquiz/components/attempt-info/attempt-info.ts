...
import { AddonModTabletQuizAttempt, AddonModTabletQuizQuizData } from '../../services/quiz-helper';
import { AddonModTabletQuiz, AddonModTabletQuizWSAdditionalData } from '../../services/quiz';
...
export class AddonModTabletQuizAttemptInfoComponent implements OnChanges {
    ...
    @Input({ required: true }) quiz!: AddonModTabletQuizQuizData;
    ...
    async ngOnChanges(changes: SimpleChanges): Promise<void> {
        ...
        if (this.attempt.sumgrades === null || !this.attempt.gradeitemmarks) {
            ...
            this.gradeItemMarks = this.attempt.gradeitemmarks.map((gradeItemMark) => ({
                ...
                grade: Translate.instant('addon.mod_tabletquiz.outof', { $a: {
                    grade: `<strong>${AddonModTabletQuiz.formatGrade(gradeItemMark.grade, this.quiz?.decimalpoints)}</strong>`,
                    maxgrade: AddonModTabletQuiz.formatGrade(gradeItemMark.maxgrade, this.quiz?.decimalpoints),
                } }),
            }));
        }
        ...
        if (this.quiz.showAttemptsMarks) {
            this.readableMark = Translate.instant('addon.mod_tabletquiz.outofshort', { $a: {
                grade: AddonModTabletQuiz.formatGrade(this.attempt.sumgrades, this.quiz.decimalpoints),
                maxgrade: AddonModTabletQuiz.formatGrade(this.quiz.sumgrades, this.quiz.decimalpoints),
            } });
        }

        const gradeObject: Record<string, unknown> = {
            grade: `<strong>${AddonModTabletQuiz.formatGrade(this.attempt.rescaledGrade, this.quiz.decimalpoints)}</strong>`,
            maxgrade: AddonModTabletQuiz.formatGrade(this.quiz.grade, this.quiz.decimalpoints),
        };

        if (this.quiz.grade != 100) {
            const percentage = (this.attempt.sumgrades ?? 0) * 100 / (this.quiz.sumgrades ?? 1);
            gradeObject.percent = `<strong>${AddonModTabletQuiz.formatGrade(percentage, this.quiz.decimalpoints)}</strong>`;
