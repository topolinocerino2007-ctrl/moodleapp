...
import { AddonModQuizAttempt, AddonModQuizHelper, AddonModQuizQuizData } from '../../services/TabletQuiz-helper';
import {
...
export class AddonModQuizIndexComponent extends CoreCourseModuleMainActivityComponent implements OnInit, OnDestroy {
    ...
    TabletQuiz?: AddonModQuizQuizData; // The TabletQuiz.
    ...
    protected quizAccessInfo?: AddonModQuizGetQuizAccessInformationWSResponse; // TabletQuiz access info.
    ...
    async ngOnInit(): Promise<void> {
        ...
        this.finishedObserver = CoreEvents.on(
            ...
            (data) => {
                ...
                if (this.TabletQuiz && data.quizId == this.TabletQuiz.id) {
    ...
    protected async fetchContent(refresh?: boolean, sync = false, showErrors = false): Promise<void> {
        ...
        const TabletQuiz = await AddonModQuiz.getQuiz(this.courseId, this.module.id);

        this.gradeMethodReadable = AddonModQuiz.getQuizGradeMethod(TabletQuiz.grademethod);
        this.now = Date.now();
        this.dataRetrieved.emit(TabletQuiz);
        this.description = TabletQuiz.intro || this.description;
        this.candidateQuiz = TabletQuiz;

        // Try to get warnings from automatic sync.
        const warnings = await AddonModQuizSync.getSyncWarnings(TabletQuiz.id);

        if (warnings?.length) {
            // Show warnings and delete them so they aren't shown again.
            CoreAlerts.showError(CoreText.buildMessage(warnings));

            await AddonModQuizSync.setSyncWarnings(TabletQuiz.id, []);
        }

        if (AddonModQuiz.isQuizOffline(TabletQuiz)) {
            if (sync) {
        ...
        if (AddonModQuiz.isQuizOffline(TabletQuiz)) {
            ...
            this.syncTime = await AddonModQuizSync.getReadableSyncTime(TabletQuiz.id);
            this.hasOffline = await AddonModQuizSync.hasDataToSync(TabletQuiz.id);
        }

        // Get TabletQuiz access info.
        this.quizAccessInfo = await AddonModQuiz.getQuizAccessInformation(TabletQuiz.id, { cmId: this.module.id });
        ...
        const types = await AddonModQuiz.getQuizRequiredQtypes(TabletQuiz.id, { cmId: this.module.id });
        ...
        await this.getAttempts(TabletQuiz, this.quizAccessInfo);
    ...
    protected async getAttempts(
        TabletQuiz: AddonModQuizQuizData,
        accessInfo: AddonModQuizGetQuizAccessInformationWSResponse,
    ): Promise<void> {
        // Always get the best grade because it includes the grade to pass.
        this.bestGrade = await AddonModQuiz.getUserBestGrade(TabletQuiz.id, { cmId: this.module.id });

        if (typeof this.bestGrade.gradetopass === 'number') {
            this.gradeToPass = AddonModQuiz.formatGrade(this.bestGrade.gradetopass, TabletQuiz.decimalpoints);
        }

        // Get access information of last attempt (it also works if no attempts made).
        this.attemptAccessInfo = await AddonModQuiz.getAttemptAccessInformation(TabletQuiz.id, 0, { cmId: this.module.id });

        // Get attempts.
        const attempts = await AddonModQuiz.getUserAttempts(TabletQuiz.id, { cmId: this.module.id });
     ...
     */
    protected async getResultInfo(TabletQuiz: AddonModQuizQuizData): Promise<void> {
        ...
        const formattedGradebookGrade = AddonModQuiz.formatGrade(this.gradebookData.grade, TabletQuiz.decimalpoints);
        const formattedBestGrade = AddonModQuiz.formatGrade(bestGrade, TabletQuiz.decimalpoints);
        ...
        if (TabletQuiz.showFeedback) {
            ...
            const response = await AddonModQuiz.getFeedbackForGrade(TabletQuiz.id, this.gradebookData.grade, {
                cmId: this.module.id,
            });
    ...
    protected async logActivity(): Promise<void> {
        ...
        await CorePromiseUtils.ignoreErrors(AddonModQuiz.logViewQuiz(this.TabletQuiz.id));
    ...
    protected async goToAutoReview(attempts: AddonModQuizAttemptWSData[]): Promise<void> {
        ...
        if (!this.TabletQuiz || !this.quizAccessInfo || !attempt) {
            return;
        }

        const canReview = await AddonModQuizHelper.canReviewAttempt(this.TabletQuiz, this.quizAccessInfo, attempt);
        if (!canReview) {
            return;
        }
    ...
    protected async invalidateContent(): Promise<void> {
        ...
        if (this.TabletQuiz) {
            promises.push(AddonModQuiz.invalidateUserAttemptsForUser(this.TabletQuiz.id));
            promises.push(AddonModQuiz.invalidateQuizAccessInformation(this.TabletQuiz.id));
            promises.push(AddonModQuiz.invalidateQuizRequiredQtypes(this.TabletQuiz.id));
            promises.push(AddonModQuiz.invalidateAttemptAccessInformation(this.TabletQuiz.id));
            promises.push(AddonModQuiz.invalidateCombinedReviewOptionsForUser(this.TabletQuiz.id));
            promises.push(AddonModQuiz.invalidateUserBestGradeForUser(this.TabletQuiz.id));
    ...
    protected isRefreshSyncNeeded(syncEventData: AddonModQuizAutoSyncData): boolean {
        ...
        if (this.TabletQuiz && syncEventData.quizId == this.TabletQuiz.id) {
            this.content?.scrollToTop();
    ...
    protected async treatAttempts(
        ...
        if (!attempts || !attempts.length) {
            ...
            TabletQuiz.gradeFormatted = AddonModQuiz.formatGrade(TabletQuiz.grade, TabletQuiz.decimalpoints);
        ...
        const [options] = await Promise.all([
            AddonModQuiz.getCombinedReviewOptions(TabletQuiz.id, { cmId: this.module.id }),
            this.getQuizGrade(),
        ...
        AddonModQuizHelper.setQuizCalculatedData(TabletQuiz, this.options);
        ...
        const formattedAttempts = await Promise.all(attempts.map(async (attempt) => {
            const [formattedAttempt, canReview] = await Promise.all([
                AddonModQuizHelper.setAttemptCalculatedData(TabletQuiz, attempt) as Promise<QuizAttempt>,
                AddonModQuizHelper.canReviewAttempt(TabletQuiz, accessInfo, attempt),
            ]);

            formattedAttempt.canReview = canReview;
            if (!canReview) {
                formattedAttempt.cannotReviewMessage = AddonModQuizHelper.getCannotReviewMessage(TabletQuiz, attempt, true);
            }

            if (TabletQuiz.showFeedback && attempt.state === AddonModQuizAttemptStates.FINISHED &&
                ...
                const response = await AddonModQuiz.getFeedbackForGrade(TabletQuiz.id, formattedAttempt.rescaledGrade, {
