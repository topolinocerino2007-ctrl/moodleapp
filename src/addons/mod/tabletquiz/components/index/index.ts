...
import { AddonModQuizAttempt, AddonModTabletQuizHelper, AddonModQuizQuizData } from '../../services/quiz-helper';
import {
...
export class AddonModTabletQuizIndexComponent extends CoreCourseModuleMainActivityComponent implements OnInit, OnDestroy {
    ...
    protected fetchContentDefaultError = 'addon.mod_tabletquiz.errorgetquiz'; // Default error to show when loading contents.
    ...
    protected async fetchContent(refresh?: boolean, sync = false, showErrors = false): Promise<void> {
        ...
        const quiz = await AddonModTabletQuiz.getQuiz(this.courseId, this.module.id);

        this.gradeMethodReadable = AddonModTabletQuiz.getQuizGradeMethod(quiz.grademethod);
        this.now = Date.now();
        ...
        const warnings = await AddonModTabletQuizSync.getSyncWarnings(quiz.id);

        if (warnings?.length) {
            // Show warnings and delete them so they aren't shown again.
            CoreAlerts.showError(CoreText.buildMessage(warnings));

            await AddonModTabletQuizSync.setSyncWarnings(quiz.id, []);
        }

        if (AddonModTabletQuiz.isQuizOffline(quiz)) {
            if (sync) {
        ...
        if (AddonModTabletQuiz.isQuizOffline(quiz)) {
            ...
            this.syncTime = await AddonModTabletQuizSync.getReadableSyncTime(quiz.id);
            this.hasOffline = await AddonModTabletQuizSync.hasDataToSync(quiz.id);
        }

        // Get quiz access info.
        this.quizAccessInfo = await AddonModTabletQuiz.getQuizAccessInformation(quiz.id, { cmId: this.module.id });

        this.showReviewColumn = this.quizAccessInfo.canreviewmyattempts;
        this.accessRules = this.quizAccessInfo.accessrules;
        this.unsupportedRules = AddonModTabletQuiz.getUnsupportedRules(this.quizAccessInfo.activerulenames);
        ...
        const types = await AddonModTabletQuiz.getQuizRequiredQtypes(quiz.id, { cmId: this.module.id });
    ...
    protected async getAttempts(
        ...
        this.bestGrade = await AddonModTabletQuiz.getUserBestGrade(quiz.id, { cmId: this.module.id });

        if (typeof this.bestGrade.gradetopass === 'number') {
            this.gradeToPass = AddonModTabletQuiz.formatGrade(this.bestGrade.gradetopass, quiz.decimalpoints);
        }

        // Get access information of last attempt (it also works if no attempts made).
        this.attemptAccessInfo = await AddonModTabletQuiz.getAttemptAccessInformation(quiz.id, 0, { cmId: this.module.id });

        // Get attempts.
        const attempts = await AddonModTabletQuiz.getUserAttempts(quiz.id, { cmId: this.module.id });
        ...
        if (this.hasQuestions) {
            if (this.attempts.length && !AddonModTabletQuiz.isAttemptCompleted(this.attempts[0].state)) {
                ...
                if (this.quizAccessInfo?.canattempt) {
                    this.buttonText = 'addon.mod_tabletquiz.continueattemptquiz';
                ...
                if (this.quizAccessInfo?.canattempt) {
                    ...
                    if (!this.preventMessages.length) {
                        if (!this.attempts.length) {
                            this.buttonText = 'addon.mod_tabletquiz.attemptquiznow';
                        } else {
                            this.buttonText = 'addon.mod_tabletquiz.reattemptquiz';
                        }
                    }
                } else if (this.quizAccessInfo?.canpreview) {
                    this.buttonText = 'addon.mod_tabletquiz.previewquiznow';
                }
            }
        }
    ...
    protected async getResultInfo(quiz: AddonModQuizQuizData): Promise<void> {
        ...
        const formattedGradebookGrade = AddonModTabletQuiz.formatGrade(this.gradebookData.grade, quiz.decimalpoints);
        const formattedBestGrade = AddonModTabletQuiz.formatGrade(bestGrade, quiz.decimalpoints);
        ...
        if (quiz.showFeedback) {
            ...
            const response = await AddonModTabletQuiz.getFeedbackForGrade(quiz.id, this.gradebookData.grade, {
                cmId: this.module.id,
            });
    ...
    protected async logActivity(): Promise<void> {
        ...
        await CorePromiseUtils.ignoreErrors(AddonModTabletQuiz.logViewQuiz(this.quiz.id));
    ...
    protected async goToAutoReview(attempts: AddonModTabletQuizAttemptWSData[]): Promise<void> {
        ...
        const canReview = await AddonModTabletQuizHelper.canReviewAttempt(this.quiz, this.quizAccessInfo, attempt);
        if (!canReview) {
            return;
        }
    ...
    protected async invalidateContent(): Promise<void> {
        ...
        promises.push(AddonModTabletQuiz.invalidateQuizData(this.courseId));

        if (this.quiz) {
            promises.push(AddonModTabletQuiz.invalidateUserAttemptsForUser(this.quiz.id));
            promises.push(AddonModTabletQuiz.invalidateQuizAccessInformation(this.quiz.id));
            promises.push(AddonModTabletQuiz.invalidateQuizRequiredQtypes(this.quiz.id));
            promises.push(AddonModTabletQuiz.invalidateAttemptAccessInformation(this.quiz.id));
            promises.push(AddonModTabletQuiz.invalidateCombinedReviewOptionsForUser(this.quiz.id));
            promises.push(AddonModTabletQuiz.invalidateUserBestGradeForUser(this.quiz.id));
    ...
    protected async sync(): Promise<AddonModTabletQuizSyncResult> {
        ...
        return AddonModTabletQuizSync.syncQuiz(this.candidateQuiz, true);
    }
    ...
    protected async treatAttempts(
        ...
        if (!attempts || !attempts.length) {
            ...
            quiz.gradeFormatted = AddonModTabletQuiz.formatGrade(quiz.grade, quiz.decimalpoints);
        ...
        const [options] = await Promise.all([
            AddonModTabletQuiz.getCombinedReviewOptions(quiz.id, { cmId: this.module.id }),
            this.getQuizGrade(),
        ...
        AddonModTabletQuizHelper.setQuizCalculatedData(quiz, this.options);
        ...
        const formattedAttempts = await Promise.all(attempts.map(async (attempt) => {
            const [formattedAttempt, canReview] = await Promise.all([
                AddonModTabletQuizHelper.setAttemptCalculatedData(quiz, attempt) as Promise<QuizAttempt>,
                AddonModTabletQuizHelper.canReviewAttempt(quiz, accessInfo, attempt),
            ]);

            formattedAttempt.canReview = canReview;
            if (!canReview) {
                formattedAttempt.cannotReviewMessage = AddonModTabletQuizHelper.getCannotReviewMessage(quiz, attempt, true);
            }

            if (quiz.showFeedback && attempt.state === AddonModTabletQuizAttemptStates.FINISHED &&
                ...
                const response = await AddonModTabletQuiz.getFeedbackForGrade(quiz.id, formattedAttempt.rescaledGrade, {
