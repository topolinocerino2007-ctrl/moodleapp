...
import { AddonModTabletQuizAttempt, AddonModTabletQuizHelper, AddonModTabletQuizQuizData } from '../../services/tabletquiz-helper';
import {
...
export class AddonModTabletQuizIndexComponent extends CoreCourseModuleMainActivityComponent implements OnInit, OnDestroy {
    ...
    TabletQuiz?: AddonModTabletQuizQuizData; // The TabletQuiz.
    ...
    protected quizAccessInfo?: AddonModTabletQuizGetQuizAccessInformationWSResponse; // TabletQuiz access info.
    ...
    protected candidateQuiz?: AddonModTabletQuizQuizData;
    ...
    protected async fetchContent(refresh?: boolean, sync = false, showErrors = false): Promise<void> {
        ...
        const TabletQuiz = await AddonModTabletQuiz.getQuiz(this.courseId, this.module.id);

        this.gradeMethodReadable = AddonModTabletQuiz.getQuizGradeMethod(TabletQuiz.grademethod);
        this.now = Date.now();
        ...
        if (AddonModTabletQuiz.isQuizOffline(TabletQuiz)) {
            if (sync) {
        ...
        this.quizAccessInfo = await AddonModTabletQuiz.getQuizAccessInformation(TabletQuiz.id, { cmId: this.module.id });
        ...
        const types = await AddonModTabletQuiz.getQuizRequiredQtypes(TabletQuiz.id, { cmId: this.module.id });
    ...
    protected async getAttempts(
        TabletQuiz: AddonModTabletQuizQuizData,
     ...
     */
    protected async getResultInfo(TabletQuiz: AddonModTabletQuizQuizData): Promise<void> {
    ...
    protected async logActivity(): Promise<void> {
        ...
        await CorePromiseUtils.ignoreErrors(AddonModTabletQuiz.logViewQuiz(this.TabletQuiz.id));

        this.analyticsLogEvent('mod_tabletquiz_view_quiz');
    }
    ...
    protected async goToAutoReview(attempts: AddonModTabletQuizAttemptWSData[]): Promise<void> {
        ...
        const canReview = await AddonModTabletQuizHelper.canReviewAttempt(this.TabletQuiz, this.quizAccessInfo, attempt);
        if (!canReview) {
            return;
        }
    ...
    protected async invalidateContent(): Promise<void> {
        ...
        if (this.TabletQuiz) {
            ...
            promises.push(AddonModTabletQuiz.invalidateQuizAccessInformation(this.TabletQuiz.id));
            promises.push(AddonModTabletQuiz.invalidateQuizRequiredQtypes(this.TabletQuiz.id));
    ...
    protected async treatAttempts(
        ...
        AddonModTabletQuizHelper.setQuizCalculatedData(TabletQuiz, this.options);
        ...
        const formattedAttempts = await Promise.all(attempts.map(async (attempt) => {
            const [formattedAttempt, canReview] = await Promise.all([
                AddonModTabletQuizHelper.setAttemptCalculatedData(TabletQuiz, attempt) as Promise<QuizAttempt>,
...
type QuizAttempt = AddonModTabletQuizAttempt & {
    canReview?: boolean;
