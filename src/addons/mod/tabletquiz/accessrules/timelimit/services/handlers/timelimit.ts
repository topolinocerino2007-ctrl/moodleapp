...
export class AddonModTabletQuizAccessTimeLimitHandlerService implements AddonModTabletQuizAccessRuleHandler {

    name = 'AddonModTabletQuizAccessTimeLimit';
    ...
    async getPreflightComponent(): Promise<Type<unknown>> {
        const { AddonModTabletQuizAccessTimeLimitComponent } = await import('../../component/timelimit');

        return AddonModTabletQuizAccessTimeLimitComponent;
    }
...
export const AddonModTabletQuizAccessTimeLimitHandler = makeSingleton(AddonModTabletQuizAccessTimeLimitHandlerService);
