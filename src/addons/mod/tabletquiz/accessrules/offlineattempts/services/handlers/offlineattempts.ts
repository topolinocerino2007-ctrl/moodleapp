...
export class AddonModTabletTabletQuizAccessOfflineAttemptsHandlerService implements AddonModTabletTabletQuizAccessRuleHandler {

    name = 'AddonModTabletTabletQuizAccessOfflineAttempts';
    ...
    async getPreflightComponent(): Promise<Type<unknown>> {
        const { AddonModTabletTabletQuizAccessOfflineAttemptsComponent } = await import('../../component/offlineattempts');

        return AddonModTabletTabletQuizAccessOfflineAttemptsComponent;
    }
    ...
    async isPreflightCheckRequired(
        ...
        const syncTime = await AddonModTabletTabletQuizSync.getSyncTime(tabletquiz.id);

        // Show warning if last sync was a while ago.
        return Date.now() - AddonModTabletTabletQuizSync.syncInterval > syncTime;
    }

}

export const AddonModTabletTabletQuizAccessOfflineAttemptsHandler = makeSingleton(AddonModTabletTabletQuizAccessOfflineAttemptsHandlerService);
